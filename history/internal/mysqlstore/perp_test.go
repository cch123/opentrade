...
func TestListPerpPositions(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{
		"user_id", "symbol", "position_idx", "side", "size", "entry_price", "margin", "leverage", "realized_pnl", "updated_at_ms",
	}).AddRow(uint64(1001), "BTC-USDT-PERP", uint32(0), int8(eventpb.Side_SIDE_BUY), "1", "100", "10", "10", "0", int64(1748505600000))
...
func TestListPerpFunding_ProducesNextCursor(t *testing.T) {
	...
	cols := []string{"perp_seq_id", "symbol", "position_idx", "funding_round_id", "funding_rate", "mark_price", "payment", "ts_unix_ms"}
	rows := sqlmock.NewRows(cols).
		AddRow(uint64(3), "BTC-USDT-PERP", uint32(1), "BTC-USDT-PERP:300", "0.01", "100", "-1", int64(300)).
		AddRow(uint64(2), "BTC-USDT-PERP", uint32(0), "BTC-USDT-PERP:200", "0.01", "100", "-1", int64(200)).
		AddRow(uint64(1), "BTC-USDT-PERP", uint32(0), "BTC-USDT-PERP:100", "0.01", "100", "-1", int64(100))
	...
	if out[0].FundingRoundId != "BTC-USDT-PERP:300" || out[0].Payment != "-1" || out[0].PositionIdx != 1 {
...
func TestListPerpLiquidations(t *testing.T) {
	...
	cols := []string{"perp_seq_id", "symbol", "position_idx", "liq_order_id", "bankruptcy_price", "mark_price",
		"closed_qty", "realized_pnl", "insurance_delta", "adl_queued", "ts_unix_ms"}
	rows := sqlmock.NewRows(cols).
		AddRow(uint64(9), "BTC-USDT-PERP", uint32(2), uint64(200), "90", "89", "1", "-10", "-2", true, int64(900))
	...
	if len(out) != 1 || !out[0].AdlQueued || out[0].InsuranceDelta != "-2" || out[0].LiqOrderId != 200 || out[0].PositionIdx != 2 {
...
func TestListPerpADL(t *testing.T) {
	...
	cols := []string{"perp_seq_id", "symbol", "position_idx", "lot_id", "adl_round", "price",
		"requested_qty", "fact_qty", "realized_pnl", "ts_unix_ms"}
	rows := sqlmock.NewRows(cols).
		AddRow(uint64(12), "BTC-USDT-PERP", uint32(1), "lot-201", uint64(2), "90", "2", "1.5", "7", int64(1200))
	...
	if len(out) != 1 || out[0].LotId != "lot-201" || out[0].FactQty != "1.5" || out[0].AdlRound != 2 || out[0].PositionIdx != 1 {
		t.Fatalf("adl = %+v", out)
	}
}
