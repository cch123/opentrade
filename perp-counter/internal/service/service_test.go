...
const (
	user1        uint64 = 1001
	user2        uint64 = 1002
	winnerUser   uint64 = 2001
	backstopUser uint64 = 9000

	user1s           = user1
	user2s           = user2
	winnerUserS      = winnerUser
	marketMakerUserS = 9001
)

func placeReq(user uint64, sym string, side eventpb.Side, price, qty, lev string, reduceOnly bool) *perprpc.PlaceOrderRequest {
...
func TestPlaceOrder_ReservesAndDispatches(t *testing.T) {
	...
	resp, err := svc.PlaceOrder(placeReq(user1, "BTC-USDT-PERP", eventpb.Side_SIDE_BUY, "100", "1", "10", false))
...
func TestPlaceOrder_InsufficientMargin(t *testing.T) {
	...
	eng.Deposit(user1, dec.New("5")) // IM needed is 10
	...
	eqd(t, eng.WalletOf(user1).Available, "5", "available unchanged on reject")
}

func TestPlaceOrder_ReduceOnlyNeedsOppositePosition(t *testing.T) {
	svc, eng, _, _ := newSvc()
	eng.Deposit(user1, dec.New("1000"))
	resp, _ := svc.PlaceOrder(placeReq(user1, "BTC-USDT-PERP", eventpb.Side_SIDE_SELL, "100", "1", "10", true))
...
func TestHandleTrade_SettlesBothLegs(t *testing.T) {
	...
	rT, _ := svc.PlaceOrder(placeReq(user1, "BTC-USDT-PERP", eventpb.Side_SIDE_BUY, "100", "1", "10", false))
	rM, _ := svc.PlaceOrder(placeReq(user2, "BTC-USDT-PERP", eventpb.Side_SIDE_SELL, "100", "1", "10", false))
...
func TestHandleTrade_ReplayGuarded(t *testing.T) {
	...
	eng.Deposit(user1, dec.New("1000"))
	eng.Deposit(user2, dec.New("1000"))
	rT, _ := svc.PlaceOrder(placeReq(user1, "BTC-USDT-PERP", eventpb.Side_SIDE_BUY, "100", "2", "10", false))
	rM, _ := svc.PlaceOrder(placeReq(user2, "BTC-USDT-PERP", eventpb.Side_SIDE_SELL, "100", "2", "10", false))
...
func TestHandleTrade_SelfTradeAppliesBothLegs(t *testing.T) {
	svc, eng, _, _ := newSvc()
	eng.Deposit(user1, dec.New("1000"))
	rM, _ := svc.PlaceOrder(placeReq(user1, "BTC-USDT-PERP", eventpb.Side_SIDE_SELL, "100", "1", "10", false))
	rT, _ := svc.PlaceOrder(placeReq(user1, "BTC-USDT-PERP", eventpb.Side_SIDE_BUY, "100", "1", "10", false))
	tr := &eventpb.Trade{
		TradeId: "t1", Symbol: "BTC-USDT-PERP", Price: "100", Qty: "1",
		MakerUserId: user1s, MakerOrderId: rM.OrderId, TakerUserId: user1s, TakerOrderId: rT.OrderId,
		TakerSide:           eventpb.Side_SIDE_BUY,
		MakerStatusAfter:    eventpb.InternalOrderStatus_INTERNAL_ORDER_STATUS_FILLED,
		TakerStatusAfter:    eventpb.InternalOrderStatus_INTERNAL_ORDER_STATUS_FILLED,
		MakerFilledQtyAfter: "1", TakerFilledQtyAfter: "1",
...
func TestCancelOrder_MarksPendingCancelAndDispatches(t *testing.T) {
	svc, eng, disp, _ := newSvc()
	eng.Deposit(user1, dec.New("1000"))
	r, _ := svc.PlaceOrder(placeReq(user1, "BTC-USDT-PERP", eventpb.Side_SIDE_BUY, "100", "1", "10", false))
	resp, err := svc.CancelOrder(&perprpc.CancelOrderRequest{UserId: user1, OrderId: r.OrderId})
	if err != nil || !resp.Accepted {
	...
	q, ok := svc.QueryOrder(&perprpc.QueryOrderRequest{UserId: user1, OrderId: r.OrderId})
...
func TestPlaceOrder_ReduceOnlyHappyPath(t *testing.T) {
	...
	if after := eng.WalletOf(user1); after.Reserved.Cmp(before.Reserved) != 0 {
