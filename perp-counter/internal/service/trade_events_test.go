...
func queryReq(user string, orderID uint64) *perprpc.QueryOrderRequest {
...
func rejectedEvt(seq uint64, user string, orderID uint64) *eventpb.TradeEvent {
...
func cancelledEvt(seq uint64, user string, orderID uint64) *eventpb.TradeEvent {
...
func acceptedEvt(seq uint64, user string, orderID uint64) *eventpb.TradeEvent {
...
func TestHandleAccepted_MarksNew(t *testing.T) {
	...
	r, _ := svc.PlaceOrder(placeReq("u1", perpSym, eventpb.Side_SIDE_BUY, "100", "1", "10", false))

	svc.HandleTradeEvent(acceptedEvt(1, "u1", r.OrderId), 0, 10)

	q, ok := svc.QueryOrder(queryReq("u1", r.OrderId))
...
func TestHandleAccepted_Idempotent(t *testing.T) {
	...
	eng.Deposit("u1", dec.New("1000"))
	r, _ := svc.PlaceOrder(placeReq("u1", perpSym, eventpb.Side_SIDE_BUY, "100", "1", "10", false))
	svc.HandleTradeEvent(acceptedEvt(1, "u1", r.OrderId), 0, 1)
	svc.HandleTradeEvent(acceptedEvt(1, "u1", r.OrderId), 0, 2) // replay
	q, _ := svc.QueryOrder(queryReq("u1", r.OrderId))
...
func TestHandleRejected_ReleasesFullIM(t *testing.T) {
	...
	eng.Deposit("u1", dec.New("1000"))
	r, _ := svc.PlaceOrder(placeReq("u1", perpSym, eventpb.Side_SIDE_BUY, "100", "1", "10", false))
	eqd(t, eng.WalletOf("u1").Reserved, "10", "reserved after place")

	svc.HandleTradeEvent(rejectedEvt(1, "u1", r.OrderId), 0, 5)
...
func TestHandleRejected_Idempotent(t *testing.T) {
	...
	r, _ := svc.PlaceOrder(placeReq("u1", perpSym, eventpb.Side_SIDE_BUY, "100", "1", "10", false))
	svc.HandleTradeEvent(rejectedEvt(1, "u1", r.OrderId), 0, 5)
	svc.HandleTradeEvent(rejectedEvt(1, "u1", r.OrderId), 0, 6) // replay: order gone, no double release
	w := eng.WalletOf("u1")
...
func TestHandleCancelled_ReleasesRemainingIMAfterPartialFill(t *testing.T) {
	...
	rT, _ := svc.PlaceOrder(placeReq("u1", perpSym, eventpb.Side_SIDE_BUY, "100", "2", "10", false))
	rM, _ := svc.PlaceOrder(placeReq("u2", perpSym, eventpb.Side_SIDE_SELL, "100", "2", "10", false))
	eqd(t, eng.WalletOf("u1").Reserved, "20", "u1 reserved after place")
...
func TestHandleTradeEvent_RecordsOffset(t *testing.T) {
	...
	eng.Deposit("u1", dec.New("1000"))
	r, _ := svc.PlaceOrder(placeReq("u1", perpSym, eventpb.Side_SIDE_BUY, "100", "1", "10", false))
	svc.HandleTradeEvent(acceptedEvt(1, "u1", r.OrderId), 3, 42)
