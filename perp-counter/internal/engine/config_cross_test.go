...
func openPosTight(e *Engine, user uint64, symbol string, side perpstate.Side, price, qty, lev string) {
	...
	e.ApplyFill(user, symbol, d(lev), perpstate.Fill{Side: side, Price: d(price), Qty: d(qty)})
}
...
func totalCash(e *Engine, user uint64) dec.Decimal {
	...
	if p, ok := e.PositionRaw(user, sym); ok {
...
func TestAdjustIsolatedMargin_AddRemoveAndGuards(t *testing.T) {
	...
	out := e.AdjustIsolatedMargin(1001, sym, "op-add", d("50"), zero)
	...
	if out := e.AdjustIsolatedMargin(1001, sym, "op-too-much", d("-55"), zero); out.Accepted {
		t.Fatalf("removal below IM requirement must reject, got %+v", out)
	}
	out = e.AdjustIsolatedMargin(1001, sym, "op-remove", d("-50"), zero)
...
func TestCrossFundingSettlesInWallet(t *testing.T) {
	...
	res, ok := e.SettleFundingUser(1001, sym, 1000, d("0.01")) // long pays 1
	if !ok || res.Payment.Cmp(d("-1")) != 0 {
		t.Fatalf("funding: %+v ok=%v", res, ok)
	}
	w := e.WalletOf(1001)
	if w.Available.Cmp(availBefore.Sub(d("1"))) != 0 {
		t.Fatalf("cross funding must hit free balance: %s", w.Available)
	}
	if p, _ := e.PositionOf(1001, sym); p.Margin.Sign() != 0 {
...
func TestCrossForceCloseAndDeficitSettle(t *testing.T) {
	...
	res, fee, ok := e.CrossForceClose(1001, sym, d("85"), 9001, d("0.001"))
	if !ok {
	...
	if bp, ok := e.PositionOf(9001, sym); !ok || bp.Size.Cmp(d("10")) != 0 || bp.Side != perpstate.SideBuy {
...
func TestAutoAddMargin_TopsUpBeforeLiquidation(t *testing.T) {
	...
	// Healthy: no fire.
	if _, fired := e.AutoAddMargin(1001, sym, d("0.005"), d("0.01"), zero); fired {
	...
	e.SetMark(sym, d("92"))
	out, fired := e.AutoAddMargin(1001, sym, d("0.02"), d("0.05"), zero)
	if !fired || out.Moved.Cmp(d("3.06")) != 0 {
		t.Fatalf("auto add: fired=%v %+v", fired, out)
	}
	if p, _ := e.PositionOf(1001, sym); p.Margin.Cmp(d("13.06")) != 0 {
	...
	e.SetMark(sym, d("88"))
	out, fired = e.AutoAddMargin(1001, sym, d("0.02"), d("0.05"), zero)
	if !fired || out.Moved.Cmp(d("0.5")) != 0 {
		t.Fatalf("auto add cap: fired=%v moved=%s", fired, out.Moved)
	}

	// Bankrupt position: no fire (liquidation path owns it).
	e.SetMark(sym, d("80"))
	if _, fired := e.AutoAddMargin(1001, sym, d("0.02"), d("0.05"), zero); fired {
...
func TestSnapshotRoundTrip_ADR0074State(t *testing.T) {
	...
	e.AdjustIsolatedMargin(1001, sym, "op-m", d("3"), zero)
	...
	p, ok := restored.PositionRaw(1001, sym)
	...
	out := restored.AdjustIsolatedMargin(1001, sym, "op-m", d("3"), zero)
	if !out.Accepted {
		t.Fatalf("cached op must replay accepted: %+v", out)
	}
	if p2, _ := restored.PositionRaw(1001, sym); p2.Margin.Cmp(p.Margin) != 0 {
