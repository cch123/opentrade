...
// TransferOutcome is the cached result of a transfer. The engine scopes the
// cache key by operation so a CompensateTransferOut credit can reuse the
// original TransferOut id without being mistaken for a retry of the debit.
...
type Engine struct {
	...
	transfers map[string]TransferOutcome // operation + transfer_id → outcome (AssetHolder idempotency, ADR-0057)
...
const (
	transferIn              transferOperation = "transfer_in"
	transferOut             transferOperation = "transfer_out"
	transferOutCompensation transferOperation = "compensate_transfer_out"

	// A NUL-prefixed namespace cannot collide with IDs generated by the asset
	// service and remains a plain string in the existing JSON snapshot map.
	// Keeping the encoded key in the map avoids a snapshot schema migration.
	transferOperationKeyPrefix = "\x00assetholder-v2\x00"
)

func transferOperationKey(operation transferOperation, transferID string) string {
	return transferOperationKeyPrefix + string(operation) + "\x00" + transferID
}

// cachedTransferLocked returns a scoped cache entry. legacyFallback also
// checks the pre-operation-aware raw transfer_id key restored from an older
// snapshot, which keeps retries of already-applied deposits/withdrawals from
// moving funds again after an upgrade.
func (e *Engine) cachedTransferLocked(operation transferOperation, transferID string, legacyFallback bool) (TransferOutcome, bool) {
	if out, ok := e.transfers[transferOperationKey(operation, transferID)]; ok {
		return out, true
	}
	if legacyFallback {
		out, ok := e.transfers[transferID]
		return out, ok
	}
	return TransferOutcome{}, false
}

// TransferIn credits the futures wallet for a saga leg (funding→futures
// deposit, ADR-0057), idempotent on the TransferIn operation and transferID. The
// second return is true when the operation was already applied (a DUPLICATED
// hit returning the original outcome).
func (e *Engine) TransferIn(user uint64, transferID string, amt dec.Decimal) (TransferOutcome, bool) {
	return e.creditTransfer(user, transferID, amt, transferIn, true)
}

// CompensateTransferOut reverses a confirmed debit using the original
// transferID. It deliberately does not treat a raw, pre-v2 snapshot key as a
// duplicate: before operation-aware keys existed that raw entry represented
// the original TransferOut, and suppressing this credit was the funds-loss
// bug this path fixes. Once applied, the scoped credit key makes retries safe.
func (e *Engine) CompensateTransferOut(user uint64, transferID string, amt dec.Decimal) (TransferOutcome, bool) {
	return e.creditTransfer(user, transferID, amt, transferOutCompensation, false)
}

func (e *Engine) creditTransfer(user uint64, transferID string, amt dec.Decimal, operation transferOperation, legacyFallback bool) (TransferOutcome, bool) {
	e.mu.Lock()
	defer e.mu.Unlock()
	if prev, ok := e.cachedTransferLocked(operation, transferID, legacyFallback); ok {
		return prev, true
	}
	...
	e.transfers[transferOperationKey(operation, transferID)] = out
	return out, false
}

// TransferOut debits free margin for a saga leg (futures→funding withdraw),
// idempotent on the TransferOut operation and transferID. An insufficient
// balance is cached as a REJECTED outcome so the same operation/id pair never
// succeeds later (the saga must use a fresh id).
...
func (e *Engine) TransferOut(user uint64, transferID string, amt dec.Decimal) (TransferOutcome, bool) {
	e.mu.Lock()
	defer e.mu.Unlock()
	if prev, ok := e.cachedTransferLocked(transferOut, transferID, true); ok {
		return prev, true
	}
	w := e.walletLocked(user)
	if e.availableToWithdrawLocked(user).Cmp(amt) < 0 {
		...
		e.transfers[transferOperationKey(transferOut, transferID)] = out
		return out, false
	}
