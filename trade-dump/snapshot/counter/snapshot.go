// Package counter serializes Counter shard state for recovery and is
// produced by trade-dump's shadow engine pipeline. Snapshots persist to
// a snapshot.BlobStore (local filesystem or shared object storage).
//
...
//
// ADR-0058 phase 1: I/O is abstracted behind snapshot.BlobStore so
// snapshots can live on shared object storage and be read by a different
// node after vshard migration.
package counter
...
// encoding.
func Save(ctx context.Context, store snapshot.BlobStore, baseKey string, snap *ShardSnapshot, format snapshot.Format) error {
...
// that as cold start).
func Load(ctx context.Context, store snapshot.BlobStore, baseKey string) (*ShardSnapshot, error) {
...
func encode(snap *ShardSnapshot, format snapshot.Format) ([]byte, error) {
	switch format {
...
func decode(data []byte, format snapshot.Format) (*ShardSnapshot, error) {
	switch format {
	case snapshot.FormatProto:
...
// which probes both.
func LoadPath(ctx context.Context, store snapshot.BlobStore, key string) (*ShardSnapshot, error) {
	...
	case ".pb":
		...
		return decode(data, snapshot.FormatProto)
	case ".json":
		data, err := store.Get(ctx, key)
		if err != nil {
			return nil, err
		}
		return decode(data, snapshot.FormatJSON)
	default:
