...
```bash
# 数据目录（放 snapshot）
mkdir -p data/counter data/match data/quote data/conditional
...
# 可选：conditional
go run ./conditional/cmd/conditional --grpc-addr=:8086 --snapshot-dir=./data/conditional
...
pkill -f "bin/counter|bin/match|bin/bff|bin/push|bin/quote|bin/trade-dump|bin/conditional|bin/history"
