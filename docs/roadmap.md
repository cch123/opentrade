...
- ~~**Counter account/balance 双层 version（方案 B）**~~ — ✅ `engine.Account.version` (user 级，任一 asset 动 +1) + `engine.Balance.Version` (per-(user,asset)，该 asset 动才 +1)；`setBalance` 单一入口原子推进两层，`putBalanceForRestore` 保留原值供 snapshot 恢复。`CounterJournalEvent.account_version` + `BalanceSnapshot.version` proto 新字段；五个 Builder（Transfer/PlaceOrder/Cancel/Settlement/OrderStatus）都填充。trade-dump `accounts` 表加 `account_version` / `balance_version` 列，upsert 按 seq_id guard 保持单调。新增 engine 侧 version 单测（Transfer / Settlement / Restore 不推进）+ snapshot round-trip 断言。所有模块 build/test/vet/race 全绿
- ~~**Counter 对 trade-event 的业务层幂等（C1：user × symbol match_seq）**~~ — ✅ `engine.Account.matchSeq map[symbol]uint64` + `LastMatchSeq` / `AdvanceMatchSeq` API；`Service.HandleTradeEvent` 提取 `evt.Meta.SeqId` 透传五个 sub-handler（Accepted / Rejected / Trade / Cancelled / Expired），sequencer fn 内 `matchSeqDuplicate` guard + 成功路径 Advance；zero-seq（legacy / 无 Meta 的 in-process 测试）bypass；snapshot schema `AccountSnapshot.LastMatchSeq` 随 account 序列化；Trade 双边 maker + taker 各自独立 guard。新增测试：重放 skip、跨 symbol 独立、zero-seq bypass、snapshot round-trip。race-clean
- **Match input topic per-symbol 化** — 当前单一 `order-event` topic + key=symbol + 默认 hash partitioner；目标 `order-event-<symbol>` per-symbol topic（或 regex 订阅），使每个 symbol 的 offset 完全独立、partition 粒度 = symbol 粒度、热加减 symbol 更清晰。[ADR-0048](./adr/0048-snapshot-offset-atomicity.md) 的"跨 symbol 取 MIN offset"逻辑在此后可退化为 per-worker 独立。需要单独 ADR（topic 命名 / 迁移双写 / 自动创建 / regex rebalance）
- ~~**Counter per-user transfer_id ring（替代全局 `dedup.Table`）**~~ — ✅ `engine.Account` 内置 256 位定长 ring（`TransferRingCapacity=256`）+ `TransferSeen` / `RememberTransfer` / `RecentTransferIDsSnapshot` / `RestoreRecentTransferIDs` API；`Service.Transfer` fast-path + sequencer 内双 check，命中后返回 `{DUPLICATED, balance=空, seq_id=0}`（**breaking**；callers 要 balance 需 `QueryBalance`）。ring 内容随 `AccountSnapshot.RecentTransferIDs` 持久化，重启后恢复；老 `DedupEntrySnapshot` 字段保留兼容 pre-ring snapshot，`dedup.Table` 本身保留但 Service 不再写。新增 engine 侧单测：fill-below-cap / overflow-evicts-oldest / duplicate-remember-noop / snapshot-round-trip / per-user-independent。JSON snapshot 体积放大是下一步 proto 化的触发点
- **Snapshot 格式 protobuf 化（可选 debug JSON）** — 四个服务（counter / match / conditional / quote）snapshot 现在都是 JSON，可读性好但体积大、解析慢。目标：默认 protobuf + `--snapshot-format=json` debug 开关（之前用户讨论过的"默认 pb、debug 时 json"）。和前面 transfer ring 一起落（触发点：JSON 快照爆体积）
- **【待调研】match→user 推送消息的合并策略** — 现状：一笔 Trade 在 counter 侧产出 **2 条** private journal（`OrderStatusEvent` + `SettlementEvent`），push 逐条 fanout → 一个 taker 吃 100 maker = **~200 条 WS 消息** 推给用户。业内参考（2026-04 调研）：**Binance** executionReport 把"订单状态 + 本笔 fill + 累积 + 手续费 + 余额字段"合成一条（100 fill = 100 条）；**OKX** `orders` channel 每笔一条 + 可选 `fills` channel（VIP5+，仍按笔）；**Bybit** `data: []` 是数组，服务端可在短窗口 batch（消息帧数可能 < fill 数）。**共同特征**：三家撮合层都**不按订单合并**（`l/L` vs `z/Z`、`fillSz` vs `accFillSz`、`execQty` vs `cumExecQty` 同时暴露 → 本笔字段存在即证据），合并仅发生在协议字段层或传输帧层。**要调研的三条路**：(A) 协议字段层合并——counter 把 OrderStatus + Settlement 合成一条 `TradeUpdate`（状态 + balance delta/after + 本笔 fill 信息），200→100，对齐 BN；(B) 传输帧层 batch——push 侧按 `(user_id, 短窗口)` 聚合多条到同一 WS 帧（`data: []`），不改 journal 语义，对齐 Bybit；(C) 分层频道——订单频道 + 明细频道分订阅，对齐 OKX，但需要 BFF/push 协议扩展。**决策点**：是否在当前规模下先做 (A)（动 journal schema + 客户端契约，收益确定）或 (B)（只动 push，侵入最小但语义/延迟打折），(C) 留到对外 API 成熟再议。**不动的部分**：match 仍按 fill 输出（审计/对账/quote K 线都依赖），trade-dump 仍按笔落库。**决策依据待收集**：(1) 实测 burst 场景下 push 帧率 / 带宽；(2) 客户端（含做市/高频）对"一笔事件 = 一条消息"的期望；(3) OrderStatus 和 Settlement 是否有"只想要其中一种"的消费方
