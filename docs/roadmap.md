...
- **Counter per-user transfer_id ring（替代全局 `dedup.Table`）** — 现状：全局 `map[transfer_id]*TransferResult` + TTL 24h，内存上界不可预测。目标：`Account.RecentTransferIDs` ring（每 user 256 个，配置化），**只记 id 不记 result**（方案 A）。命中语义降级：`{DUPLICATED, balance=空}`，client 重试要 balance 自己再 `QueryBalance`（**breaking**，BFF 文档要同步）。**必须进 snapshot**（raw id，人眼可读）—— 10k user × 256 × 50B ≈ 128MB raw / JSON 2-3× 放大到 ~400MB，是**推进 proto snapshot 化的强触发点**，建议和 proto 化一批落
