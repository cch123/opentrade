...
- **Counter account/balance 双层 version（方案 B）** — `Account.Version`（user 级，任一 asset 变化 +1，给缓存失效 / 乐观锁做抓手）+ `Balance.Version`（per-(user,asset)，用于事件幂等 guard）；snapshot schema 再升版、trade-dump `accounts` 投影表加 version 列。**前置条件：**下一项的 handler guard 依赖它
- **Counter 对 trade-event 的业务层幂等（C1：user × symbol match_seq）** — 现在 `handleTrade` / `handleAccepted` / `handleCancelled` 没有任何 in-handler 幂等 guard，重放 trade-event 会重复扣款 / 重复结算；ADR-0048 的 offset seek 是结构性防御，不是业务幂等。做法：`Account.LastMatchSeq map[symbol]uint64`，handler 入口 `if evt.match_seq <= acc.LastMatchSeq[evt.symbol] { skip }`，成功后推进。**必须进 snapshot**（否则重启 seen-set 清零 dedup 失效）。选 C1 不选 C2（user×asset×symbol）：trade-event 影响 base/quote 同步，symbol 层 guard 已精确，C2 冗余且 snapshot 开销放大。match_seq 比 `Balance.Version` 更自然（match 天生带 per-symbol seq，不需要感知 counter state）
