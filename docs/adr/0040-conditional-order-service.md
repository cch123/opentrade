# ADR-0040: 条件单独立服务（stop-loss / take-profit）
...
（MARKET 有 BFF 滑点保护路径，ADR-0035）。客户一个明确的缺口是**条件单**：
...
户状态机**。条件单的触发逻辑需要一个新的 **行情观察者**。

## 决策 (Decision)

### 1. 独立 `conditional` 服务，订阅 Quote 的 `market-data`

新增微服务 `conditional`：

- gRPC 服务 `ConditionalService.{Place,Cancel,Query,List}`；外部通过 BFF
  REST `/v1/conditional*` 到达
...
- 触发时下内部 PlaceOrder，Counter 按常规校验 + freeze；如果余额不足，
  Counter 返回 `FailedPrecondition`，conditional 把该条置为
...
- `client_conditional_id` 作为用户侧幂等键：重复 Place 返回原 id +
  `accepted=false`
- 内部触发 Counter 时，`client_order_id = "cond-<id>"` 固定派生 —— 重启
...
- 引擎状态转换不靠乐观标记：从 PENDING 拉出 → 释锁 → 调 Counter → 回锁
  ...
  会再 fire；Counter 看到同样的 `cond-<id>` 客户端订单 id 走 dedup 返回上
  次的 `order_id`；最终状态对齐
...
- UX 好很多（下单即锁资金）
- 但要么扩 Counter 的 transfer proto（新 reserve-for-conditional type +
  biz_ref_id 语义），要么让 conditional 发 FREEZE transfer 再 fire 前 UNFREEZE
  + PlaceOrder（两步有竞态：竞速的订单可能吃光余额）
- **scope 太大**；先把"触发链路"这条骨干跑通，冻资金作为 MVP-14b 迭代

### E. 把触发统一丢 Counter 去轮询
- 即：Counter 拉 market-data 周期性扫 pending conditionals
- 本质上 = A，同样拒绝

## 理由 (Rationale)

- **职责分离**：Counter 永远不观察行情；Quote 只发布；conditional 是纯粹
...
- 新服务完全自包含（proto + cmd + internal + tests），不动 Counter /
  Match / Quote / Push
- REST 侧一致：`/v1/conditional` 跟 `/v1/order` 同风格，client_conditional_id
  也是 `client_order_id` 的翻版
- 本地 snapshot 让重启不丢 pending 条件单（相比"每次从 earliest 重扫"）

### 负面 / 代价

- 新服务要运维一根——日志、监控、部署 pipeline（条件单量级通常远小于
  真实订单，压力本就很低）
- 无 freeze 的 UX 折扣：极端行情下触发瞬间发现余额已耗尽；MVP 文档说明
- 触发延迟 = Match→Quote→market-data partition fetch→conditional 引擎
...
- 触发后不再订阅 order-event 跟踪结果 —— 用户通过 WS / `/v1/order/{id}`
  自行观察内部订单；conditional 只负责 "已成功下发内部订单" 状态

## 实施约束 (Implementation Notes)

### Proto

`api/rpc/conditional/conditional.proto`：

- enum `ConditionalType`: STOP_LOSS / STOP_LOSS_LIMIT / TAKE_PROFIT /
  TAKE_PROFIT_LIMIT
- enum `ConditionalStatus`: PENDING / TRIGGERED / CANCELED / REJECTED
- 四个 RPC：Place / Cancel / Query / List
- `Conditional` 消息包含 stop_price、limit_price、qty、quote_qty、tif、
...
- `bff/internal/client/conditional.go`：narrow 客户端 + Dial
- `bff/internal/rest/conditional.go`：四个 handler + JSON ↔ proto 转换
- `bff/cmd/bff/main.go`：`--conditional` flag 指定 gRPC 端点；空则
  `/v1/conditional*` 返回 503（和 ADR-0038 同策略）

### 失败场景

| 情况 | 结果 |
|---|---|
| Counter 不可达 / timeout | 条件单 → REJECTED + reject_reason |
| Quote 挂 / 无 PublicTrade 流入 | 条件单一直 PENDING（不触发）——和 Push / BFF-marketcache 共享故障 mode |
| conditional 崩溃 | 下一次启动从 snapshot + offset 续；pending 还在，未 commit 的 triggered 会因 Counter dedup 收敛 |
| snapshot 磁盘满 | log error，continue；下一次 tick 重试；最坏情况 = 崩溃时丢最近 n 秒 pending |
| 同一 price 多条 conditionals 同时 fire | 都过 RPC；Counter 各自校验；互不影响（每个有独立 `cond-<id>` client order id） |
...
- **MVP-14b**：资金预留。最干净的实现是在 Counter 里新增
  ...
  RPC；conditional 在 Place 时 Reserve，触发前 Release（或 PlaceOrder
  带 `use_reservation_id` 原子替换）；cancel/expire 走 Release
- **MVP-14c**：HA。etcd leader election + cold standby（照搬 ADR-0031 的
  Counter/Match 模型）
- **OCO**（One-Cancels-Other）：两个挂钩在一起的 conditionals，任一 fire
...
- 实现：
  - [api/rpc/conditional/conditional.proto](../../api/rpc/conditional/conditional.proto)
  - [conditional/cmd/conditional/main.go](../../conditional/cmd/conditional/main.go)
  - [conditional/internal/engine/engine.go](../../conditional/internal/engine/engine.go)
  - [bff/internal/rest/conditional.go](../../bff/internal/rest/conditional.go)
