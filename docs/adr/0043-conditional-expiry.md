...
ADR-0040 实现的条件单没有过期机制。用户下一个止损后，如果不手动取消，
...
tif 语义。考虑到条件单的触发时间不可控，显式 `expires_at_unix_ms`
比 TIF-based 表达更直接。
...
- REST `POST /v1/conditional` 增加 `expires_at_unix_ms`（JSON int64，0
  表示无 TTL）
- 响应 / Query / List 的 Conditional 对象携带 `expires_at_unix_ms`
- 状态码字符串新增 `expired`
...
- 复用订单的 TIF enum 思路
- 缺点：`TIF` 在条件单里是"触发后下内部单的 TIF"，语义已被占用；再
  塞入过期时间会混淆
- 拒绝
...
- Sweeper 每 5s 扫一次 PENDING。单用户几千个条件单也是毫秒级遍历，上
...
- `api/rpc/conditional/conditional.proto`：加 `expires_at_unix_ms` 两处 +
  `CONDITIONAL_STATUS_EXPIRED`
- `conditional/internal/engine/engine.go`：
  - `Conditional.ExpiresAtMs` 字段
  - `buildConditional` / `Place` 接受并验证
  - `SweepExpired(ctx) int` 方法
  - `ErrExpiryInPast`
- `conditional/internal/engine/wire.go`：`ToProto` 带 ExpiresAtUnixMs
- `conditional/internal/snapshot/snapshot.go`：`ConditionalSnap.ExpiresAtMs`
  序列化
- `conditional/cmd/conditional/main.go`：`--expiry-sweep` flag +
  `runExpirySweeper` goroutine 挂在 primary 里
- `conditional/internal/server/server.go`：错误映射 `ErrExpiryInPast` →
  InvalidArgument
- `bff/internal/rest/conditional.go`：`placeConditionalBody.ExpiresAtUnixMs`
  透传；`conditionalStatusLabel` 加 `"expired"`
...
| sweep 运行中 Counter gRPC 挂掉 | RPC err log；conditional 侧仍然是 EXPIRED；reservation 留在 Counter。下一次 cancel / restart 手动处理（和 crash 场景一致） |
...
监控指标（未来埋）：`conditional_expired_total`、`conditional_expiry_sweep_duration`。

## 未来工作 (Future Work)

- **Rolling conditionals**：EXPIRED 自动续下一条（可选字段
  `renew_after_expired=true`）
- **客户端 TTL 默认**：BFF 层给未传 `expires_at` 的请求兜底，比如默认
  7 天 TTL
- **过期事件流**：对接 Push，让用户 / 审计拿到 `conditional.expired`
  通知
...
- ADR-0042: Conditional HA（primary 上的 sweeper）
- 实现：
