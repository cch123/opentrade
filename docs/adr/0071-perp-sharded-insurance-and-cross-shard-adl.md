...
- **保险基金会碎掉**：每个 shard 的 `insurance[BTC]` 只看得到**自己 owned 用户**在 BTC 上的强平盈余/亏损，全局池被切成 per-`(shard, symbol)` 碎片。这与"保险池应当是全局的"（单 symbol 所有用户共用一个吸损池）直接冲突。
- **ADL 跨进程**：按 user 分片后，某 symbol 的盈利对手仓散落在**所有 shard**，没有任何单个 shard 能独立排出全局 ADL 名单；0070 的"同进程同 snapshot 内派发"前提失效。

本 ADR 决定：分片后**引入一个全局风控协调器**（fund 余额 + ADL 排名/决策 + 托管周转金账务的单一 owner），**perp-counter shard 退化为持仓权威 + 执行器**。这看似违背 [ADR-0068 §C / 0070 §E](./0070-perp-liquidation-hardening.md) "不拆独立强平服务"，实则**恪守了 §E 的正确性论证**——§E 反对的是"独立服务读远程仓位、然后自己动手"（TOCTOU），本 ADR 的协调器**只决策、不动仓**，所有仓位 mutation 回到 owning shard 的 per-user sequencer 内、**带版本戳复核**后执行（§决策 2）。
...
参考实现（均已读源、逐条核对代码）：
- **`bybit-leaked/trading`（正面范本）**：trading_service **按 user 分片**（`globalvar.ShardName`），ADL **决策在外部**、trading 只执行（`dispatcher/internal/process.go:119` 收 `AdlExecuteReq`），靠 **`OrigCrossSeq` 版本戳**复核（`liqadlbiz/adl_execute.go:82`、`liq_execute.go:39`），保险池是**独立全局服务**（per-coin 全局 + per-symbol 每日配额，`risk_pool_result_dto.proto`），且走**先托管、借周转金、甩卖、ADL 兜底**模型。
- **`cryptofabric/unimargin-liquidate-server`（反面教材，0070 §E 已批）**：单 Raft 组、不分片、**持仓零存**，ADL 对手方从 quote-server 的**最终一致影子副本**现读，**无版本号守卫**（只有本地咨询锁 + reduceOnly）——正是"集中决策者拿过期远程读直接动手"的活标本。
...
|---|---|---|
| `全局风控协调器`（perp-risk） | fund 余额 + ADL 排名/决策 + 托管周转金账务的**单一全局 owner**；只决策不动仓 | Bybit "ADL Service + 保险池服务"合体 / BN risk engine |
...
| `版本戳`（pos_seq guard） | 协调器决策时所基于的仓位版本；执行端比对，已推进则拒绝任务 | **Bybit `OrigCrossSeq`**（命根子）/ 0070 `last_match_seq`/`adl_round` |
| `InsuranceDelta` | 单笔强平/接管释放或穿透的、流入/流出 fund 的签名增量 | — （已存在于 `PerpLiquidationEvent`） |
| `托管`（takeover） | 系统账户接管被强平仓位库存，事后慢慢甩卖 | Bybit 强平托管 `TakenOverPosition` / 0070 §3 backstop 升级 |
| `周转金`（working capital） | 托管库存期间向全局池**借**的持仓资金，甩卖结束**还**（盈余填池/亏损抽池） | Bybit `RiskPoolDTO.borrow_or_lend` |
...
**单实例下这是对的**：一个进程吃下全部分区、结算全部用户，"per-symbol in-engine 标量"恰好就是全局 per-symbol 池。**问题只在打开分片那一刻显现**（见 §范围声明）。所以本 ADR 不是修当前 bug，是"打开分片"工作流必须先落的地基。

### 参考实现对比（已读源、核对代码）

| 维度 | unimargin（反面教材） | **bybit（范本）** | 本 ADR 取舍 |
|---|---|---|---|
...
| ADL 执行者 | liquidate-server 拼对单丢给 trade-server | **owning shard 执行**（持仓权威） | 跟 bybit：回 owning shard sequencer |
| TOCTOU 守卫 | 本地咨询锁 + reduceOnly（**无版本号**） | **`OrigCrossSeq` 版本戳**，shard 不符即拒 | 跟 bybit：版本戳握手（= 0070 的 `last_match_seq`/`adl_round`） |
| 保险基金 | **无**（系统账户透支兜底） | **独立全局服务**，per-coin 全局 + per-symbol 配额 | 跟 bybit：全局 fund（协调器持有）+ 配额 |
| fund 模型 | n/a | 借/还**周转金**（先托管→借→甩卖→还），Kafka 异步结算 | 跟 bybit：托管 + 周转金，credit 异步 fold |
| ADL 排名 | 裸 max 未实现盈利、每轮一个 | 杠杆/盈利加权（ADL 灯 1-5），rank 在 shard 维护 | 沿用 0070 `adl_score`，rank 由 shard 报、协调器排 |

**结论**：bybit 同时回答了"insurance 该独立（独立全局池服务）"和"ADL 决策与执行分离（外部决策 / 分片执行）"两个问题，且用 `OrigCrossSeq` 给出了 unimargin 缺失的那块正确性拼图。本 ADR 把 bybit 的三角色映射到 OpenTrade，并把 bybit 拆得很散的"ADL 决策者 + 保险池出纳"在 MVP 阶段**合并为一个协调器**（量级低、同属全局，见 §备选方案 E）。
...
```
        ...
        │   ├─ ADL 排名 + 决策（deficit→减谁/多少）    │   只产出"带版本戳的任务"，不动仓
        │   └─ 托管周转金账务（借/还全局池）           │
        └───────────────┬──────────────────────────┘
...
- **宿主选择**：协调器既可新建独立 `perp-risk` 服务（**失败半径隔离**，对齐 0068 §C 看重的故障隔离——风控风暴炸了不连累 mark-price 发布），也可折叠进 `markprice`（它已是单实例 + cold-standby、已每 tick fanout 到所有 shard、已有全局视角，见 [0069 §备选方案 E](./0069-external-composite-index-price.md) 的"fold vs split"先例）。**MVP 倾向独立 `perp-risk`**（隔离 > 省一个服务），最终拍板列开放问题。

### 2. 版本戳握手（本 ADR 的命根子，缺它就是 unimargin）

协调器派发给 shard 的**每一个** liq/takeover/adl 任务，必须携带它**决策时所基于的仓位版本**（`pos_seq` = 0070 已有的 `last_match_seq`；ADL 另带 `adl_round`）。owning shard 在自己的 per-user sequencer 内执行前**比对版本**：

```
协调器：基于"快照 @ pos_seq=N"决策 → 派发 task{user, symbol, qty, price, pos_seq:N, adl_round:R}
shard ：进 user 的 sequencer → if 当前仓位.pos_seq != N → 拒绝（仓位已被在途成交推进，决策过期）
                              → else 执行 mutation，pos_seq++，幂等水位记 adl_round=R（重放不重复）
```

- 这正是 bybit `refPz.Latest().CrossSeq != req.OrigCrossSeq → 拒绝`（`liq_execute.go:39` / `adl_execute.go:82`）。**协调器的决策允许基于稍旧的全局视图，正确性由执行端的版本复核兜底**——这把"跨服务读 → 动手"的 TOCTOU 窗口关死，恪守 0068 §C/0070 §E 的论证。
...
### 4. 托管 + 周转金 + 跨 shard ADL（取代"挂破产价单靠盘口"）

把 0070 §3 backstop 升级为 bybit 的 **先托管、借周转金、甩卖、ADL 兜底** 模型：

```
shard：单仓破 MM → 0070 救援阶梯（降档/撤单/加保证金/部分强平）→ 仍不行 → 整仓接管：
        用户仓位即刻平给 backstop 系统账户（用户出场），backstop 记反向库存
        ── 发 takeover 事件 + InsuranceDelta（接管时的权益缺口）──▶ 协调器
协调器：向全局池为该托管库存"借"周转金（受 per-symbol 配额约束）
        backstop 库存按 bankruptcy_price 甩卖（reduce_only 挂 Match）
        ◇ 甩卖吃不下 / 配额不足 / fund 转负(deficit>0) ?
          否 → 慢慢甩完 → "还"周转金（甩卖盈余填池 / 穿仓抽池）
          是 → 触发跨 shard ADL（见下）
```
...
3. 协调器把每个 ADL 任务**带版本戳**派发到**对手仓 owning shard**；shard 在对手用户 sequencer 内**复核仍反向/盈利/存在 + pos_seq 匹配**（§2），再按 bankruptcy_price 平掉、赢家让利落账、发 `PerpAdlEvent`、`adl_round` 幂等。
4. 补足 deficit 则停；不足继续下一候选。

> 关键：协调器**永不直接改仓**，只发版本戳任务；候选名单滞后无害，因为执行端复核兜底。这与 unimargin"集中读 quote 影子副本直接拼对单"的本质区别，就在第 3 步的版本戳复核。

## 备选方案 (Alternatives Considered)

### A. 保险基金留 perp-counter in-engine per-shard（现状 / 0068 §9）
- 单实例下正确（in-engine 标量 == 全局池）。但**打开分片即碎成 per-`(shard,symbol)` 片**，单 symbol 的吸损池被人为切开，与"全局池"语义冲突，且各 shard 各自判 deficit 会重复/错误触发 ADL。**否决**（仅作单实例 MVP 的退化形态保留）。
...
- 若按 symbol 分片，则 per-symbol fund 和 ADL 对手方都落同一 shard，0070 的"同进程"假设每 shard 内仍成立，无需协调器。**否决**：按 symbol 分片**断掉 cross-margin**（一个用户跨 symbol 的仓位会被劈到不同 shard，算不了账户级保证金），而 [0068 §3 的"一律走 pool 接口"](./0068-usdt-linear-perp.md) 硬约束正是为 cross-margin 铺路。**user 分片是为 cross-margin 必须付的代价，fund/ADL 走全局协调器是其必然结果**——鱼和熊掌。

### D. unimargin 式：集中决策者读远程仓位直接动手
- liquidate-server 现读 trade-server/quote-server 仓位、自己拼对单。**否决**：对手方侧无版本号守卫（只有本地咨询锁 + reduceOnly），决策基于滞后影子副本、执行不复核——0068 §C 批判的 TOCTOU，代码已坐实是 race。本 ADR 取其"集中决策"形、**弃其"直接动手"**，用 §2 版本戳把决策与执行解耦且复核。
...
- 纯 per-symbol：回到"碎片池"，单 symbol 缓冲薄。纯全局无配额：单合约爆仓可抽干全局池、殃及所有合约。**选 per-coin 全局 + per-symbol 每日配额**（bybit 形态）：既满足"池全局"，又用配额堵住单合约抽干的尾部风险。（具体百分比如"BTC 100% / 其余 20%"是配置项，bybit 那份分析里的数字仅作示意。）

## 理由 (Rationale)

1. **分片是既定前提**（0068 §A / 0010），cross-margin 又要求按 user 分片（备选 C），所以 fund/ADL 的全局性**只能**靠协调器解决——这不是"要不要拆服务"的偏好，是分片拓扑的逻辑后果。
2. **协调器不破 0068 §C/0070 §E 的正确性论证**：§E 反对的是"读远程仓位直接动手"，协调器只决策、shard 带版本戳执行+复核，TOCTOU 窗口关死（§2）。bybit 用 `OrigCrossSeq` 在生产中验证了这条路；unimargin 缺它正是反证。
...
5. **托管 + 周转金比"挂破产价单靠盘口"鲁棒**：把"用户即刻出场"与"库存慢慢甩"解耦，正面解 0070 缺口 A（行情急跌盘口薄→最该平时平不掉）；fund 当周转金放贷而非被动累加，ADL 真正退为最后一步。

## 影响 (Consequences)

### 正面
- 保险池真正全局（per-coin），单 symbol 吸损池不再被分片切碎；deficit 判定单一权威，不重复/误触发 ADL。
- ADL 在分片拓扑下正确闭合，且沿用版本戳保证不踩 unimargin 的 race。
- 协调器低 QPS + 单实例 + cold-standby，故障与撮合热路径隔离。
- 托管周转金模型让薄盘下强平也保证闭合（接 0070 缺口 A）。
...
- backstop 系统账户的库存本身仍落某个 shard（它也是个 user），链下对冲流程沿用 0070。
...
- **ADL 候选来源**：MVP 选 (a) 各 shard 周期上报 owned 高分候选给协调器（新鲜、量可控）；(b) 查 perp 持仓投影作兜底。无论哪个，**执行端版本戳复核兜住滞后**。
...
12. **全局 fund 单一权威**：分片后 fund 余额只有协调器持有；shard 只产出 `InsuranceDelta`，不持有权威余额、不基于局部 fund 判 deficit。（单实例 MVP 是其退化等价形态——**多实例未上线前，须在 perp-counter 启动处 guard "分片数>1 且无协调器 = 拒绝启动"**，防静默碎裂。）
13. **版本戳握手（命根子）**：协调器派发的每个 liq/takeover/adl 任务带 `pos_seq`（+`adl_round`）；owning shard 执行前比对，不符即拒。绝不允许"无戳直接执行"。（= bybit `OrigCrossSeq`；缺它就是 unimargin 的 race。）
...
16. **全局池 + per-symbol 配额**：单 symbol 当日借出 ≤ 配额；超配额走"减小甩卖量 / 优先 ADL"，不静默穿配额。
17. **协调器决策幂等 + 可恢复**：fund fold、托管借据、ADL 在途全随协调器 snapshot 绑 offset；重启 replay 不重复借/重复减仓。

### 测试要点
- 单测：fund fold 守恒（credit/debit/借还周转金累计 = 全局余额）；配额三取 min + 每日重置；版本戳复核（戳过期→拒、戳匹配→执行一次）；`adl_round` 幂等重放。
- 集成：开仓 → 多 shard 各有对手仓 → 某 user 穿仓接管 → 协调器借周转金甩卖 → 甩不动 → **跨 shard ADL 减不同 shard 的对手盈利仓** → fund 收敛；协调器宕机中途 → snapshot+replay 一致（借据/ADL 队列不丢不重）。
...
markprice    perp-counter shard X (user A sequencer, 本地无 TOCTOU)        协调器 perp-risk
  ...
  │             │ （A 已出场, 不等协调器, 无条件闭合）                            │ （credit 异步, 最终一致）
```

#### 图 2 — 托管 + 周转金借/还（薄盘保证闭合）
...
shard X (user A)              协调器 perp-risk (全局 fund + 配额)        Match / backstop
  ...
  │                                                         │ backstop 库存 reduce_only @ bankruptcy ─►│ 甩卖
  │                                                         │◄────────── 成交 / 吃不下 ───────────────┤
  │                                                         │ ◇ 甩完? → 还周转金(盈余填池/穿仓抽池)
  │                                                         │ ◇ 吃不下/配额不足/deficit>0 → 进图 3 ADL
```

#### 图 3 — 跨 shard ADL（协调器决策 / shard 版本戳执行，唯一跨 shard 协调点）

```
协调器 perp-risk (穿仓兜底决策)        版本戳派发           shard Y/Z (对手盈利方 user B/C)
  │ deficit = -global_fund(coin)        │                    │
  │ 候选 = 反向盈利仓 by adl_score       │                    │   （候选可来自 shard 上报 / 持仓投影, 允许滞后）
  │ 选名单(只读, 不动仓) ───────────────►│ task{B, sym, qty,   │
  │                                      │   price, pos_seq:N,─►│ 进 B 的 sequencer:
  │                                      │   adl_round:R}      │   ◇ B.pos_seq == N 且 仍反向/盈利/存在 ?
  │                                      │                     │     否 → 拒绝(决策过期, 等下轮)
...
- **per-symbol 配额具体策略**：百分比、每日重置时点、BTC 特权与否；配额耗尽时"减甩卖量 / 优先 ADL / 更激进价"的次序。
...
- 参考实现（已读源、核对代码）：
  ...
  - `cryptofabric/unimargin-liquidate-server`（反面教材）：单 Raft 组、持仓零存、`QuoteService.getMaxProfitRatePosition` 读 quote 影子副本、对手方侧仅本地咨询锁 + reduceOnly（无版本号）——0068 §C/0070 §E 批判的 TOCTOU 活标本。
