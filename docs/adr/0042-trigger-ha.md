# ADR-0042: 触发单服务 HA（cold standby）
...
`auto` 下两个 trigger 进程共用一个 etcd key：
...
- 打开 gRPC `TriggerService` 监听 `--grpc-addr`
- 订阅 `market-data` Kafka topic 并触发触发单
...
2. **Pending / terminal triggers**：snapshot 里的 JSON；primary 启动
   时 `snapshot.Restore` 灌进 engine
3. **Counter 上的 reservations**（MVP-14b / ADR-0041）：归 Counter 管，
   两个 trigger 实例共享同一份 Counter reservation 表。primary 切换
...
Trigger 不往 counter-journal 写事件（ADR-0040 §3），没有 Kafka
...
trigger 里退化为"浪费几次 RPC，不产生错误"，远弱于 Counter/Match
的 trade-event 双写风险，不需要额外 fencing。

## 备选方案 (Alternatives Considered)

### A. Active-active 多实例 + partition 分片
像 Push（ADR-0033）一样按 user / symbol 分片，每个 trigger 实例只处
理一部分。

- 优点：无 failover 延迟，规模扩展线性
- 缺点：
  - 需要按 user_id（或 trigger_id）分 partition，市场数据 fan-out 要做 fil
    ter（和 ADR-0033 一样）
  - 触发要跨分片协调（同一用户的两条 trigger 触发可能落到不
    同实例）
  - 实现量远大于 cold standby
- 结论：触发单量级不会到单机撑不住的程度（触发单总量 < 订单总量），先
  不做

### B. 不做 HA，靠"重启快"
- 优点：零代码
- 缺点：重启期间 pending 触发单无人照看，用户止损单可能"睡过去"最糟糕
  的几秒
- 结论：MVP-14a/b 就是这样，14c 的目的就是补

### C. 直接写 counter-journal 事件 + 事务 fencing
- 把 trigger 的状态变更也写进 Kafka
- 优点：和 Counter 一样严格的分布式一致性
- 缺点：大改 proto + trade-dump，产生大量对业务无意义的 journal 事件
- 结论：trigger 的操作 idempotency（reservation_id + client_order_id）已经
  足够，不需要这层重叠保护

### D. 以 Quote 为参照，单实例不做 HA
Quote 也只是单实例（ADR-0036 没做 HA）。

- Quote 和 trigger 差别：Quote 挂了下游 Push/BFF-marketcache 只是
  暂停更新，不产生"应该触发但没触发"的后果；trigger 挂了 = 用户止
  损失灵，金钱直接损失
- 结论：trigger 优先级高于 Quote，HA 必要

## 理由 (Rationale)

- **复用即 win**：`pkg/election` 已经在 Counter/Match 生产级验证过；
  trigger 的进程结构本来就和 Counter 对齐；直接套用
...
- primary crash 窗口内，新 primary 几秒内接管，pending 触发单照常触发
...
- 两个 trigger 进程共享同一个 Kafka consumer group id →
...
- `--ha-mode=auto` 引入了 etcd 依赖。trigger 的可用性下限现在是
...
--election-path    /cex/trigger/leader
...
- **partition 分片 active-active**：看 trigger 量级是否撑到单机触发
...
- 实现：[trigger/cmd/trigger/main.go](../../trigger/cmd/trigger/main.go)、[pkg/election/](../../pkg/election/)
