# ADR-0044: 条件单 OCO（One-Cancels-Other）
...
ADR-0040 只支持独立（standalone）条件单，需要用户自己写两条然后指望触
...
- 每腿复用既有 PlaceConditional 结构 —— MVP-14a/b/d 的全部能力
...
`client_conditional_id` 级 dedup 正交 —— 两者都可以独立工作。
...
`ConditionalSnap.OCOGroupID` + 顶层 `OCOByClient` map 均入
snapshot。Graceful restart 后级联逻辑、dedup 均继续工作。

## 备选方案 (Alternatives Considered)

### A. 两条独立 PlaceConditional + 后置 "link group" RPC
- 优点：proto 最小
...
- 优点：proto 最朴素
- 缺点：我们现有 4 种 ConditionalType 已经覆盖 BN 的所有腿形态，直接
  拼成 legs[] 不需要新字段
...
- **幂等两层**：per-leg `client_conditional_id` + group `client_oco_id`
...
- 既有 standalone 条件单行为不变（`OCOGroupID == ""` 跳过 cascade 逻
  辑，零开销）
- PushFeed 的条件单推送只需要带上 `oco_group_id`，客户端就能把腿对齐
...
- `api/rpc/conditional/conditional.proto`：
  ...
  - `Conditional.oco_group_id = 18`
- `conditional/internal/engine/engine.go`：
  - 新字段 `Conditional.OCOGroupID` + `Engine.ocoByClient`
...
- `conditional/internal/engine/wire.go`：`ToProto` 带 `OcoGroupId`
- `conditional/internal/snapshot/snapshot.go`：`ConditionalSnap.OCOGroupID` +
  `Snapshot.OCOByClient`；`Restore` 调用新 `engine.SetOCOByClient`
- `conditional/internal/service/service.go` + `server.go`：`PlaceOCO` 方
  法 + gRPC handler + error mapping
- `bff/internal/client/conditional.go`：接口加 `PlaceOCO`
- `bff/internal/rest/conditional.go`：`POST /v1/conditional/oco` handler
  + `placeOCOBody` 结构
...
| 级联 Release 网络失败 | Debug log；conditional 侧状态已 CANCELED；reservation 遗留在 Counter 直到 operator 清理 |
