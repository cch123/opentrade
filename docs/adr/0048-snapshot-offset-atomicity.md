...
所有三个服务（本 ADR 改 match/counter；quote / trigger 将后续对齐）的 Capture 调用点必须严格遵守：
...
- trigger 走这条（[ADR-0047](./0047-trigger-long-term-history.md) 非事务 producer + 队列满即丢 + WARN 日志）
- trigger 的输出是审计用的 journal，业务链路不依赖其完整性
- counter / match 的输出是账户 / 撮合主链路，丢一条就是错账 / 残单 → 不可接受
- 结论：trigger 保持现状；counter / match 必须走 flush barrier
...
- **和 quote/trigger 对齐**：四个状态服务用同一恢复范式，新人一次学清
...
- 实现：
  ...
  - [counter/internal/tradeevent/consumer.go](../../counter/internal/tradeevent/consumer.go)
