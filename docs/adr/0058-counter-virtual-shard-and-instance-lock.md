...
本 ADR 落地需分阶段推进。第一阶段只做 snapshot 共享存储，是所有后续阶段的前置 enabler：

1. **阶段 1**：snapshot 共享存储（fs + s3 后端），shard 模型不动 ← **立即开始**
2. **阶段 2**：etcd assignment schema + coordinator（选主 + 初始分配）
3. **阶段 3**：counter node 按 assignment 动态启停 vshard 的消费 / 生产
4. **阶段 4**：Kafka topic 重建（trade-event / counter-journal 改 256 partition）+ Match 侧 partition key 对齐
5. **阶段 5**：BFF 路由改造（watch etcd + epoch 错误重试）
6. **阶段 6**：冷切迁移协议 + 运维 API
7. **阶段 7**：自动 failover（node lease 过期触发重分配）

每个阶段独立 merge、独立验证。阶段 1-2 阶段 3 之间的 counter 仍然是 10 固定 shard，仅 snapshot 位置变了；阶段 3 起切换到 vshard 模型。
