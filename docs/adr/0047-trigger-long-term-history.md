# ADR-0047: 触发单长期历史（trigger-event + history projection）
...
MVP-14a~14f 把触发单服务跑起来了，但它只在内存里保留 `TerminalHistoryLimit`
轮终态（默认几百个），重启 / 滚动升级后就清零。客户端查"三个月前那笔
stop-loss 最终怎么了"这条需求没有权威来源。

ADR-0046 起了 `history` 服务做只读查询，但它读的是 trade-dump 投影的
`orders` / `trades` / `account_logs` 三张表，触发单完全没上车 —
因为 trigger 服务根本不写 Kafka（ADR-0040）。

可选路线：
- **A. trigger 每次 Query/List 回源 MySQL**：让 trigger 服务
  自己读/写 MySQL。这会把 trigger 变成又是热路径状态机，又是读
  服务，违反 ADR-0046 的分层。
- **B. 把触发单快照周期写 MySQL**：trigger 定期把全量状态 dump
  到 MySQL。延迟粗，写量大（全量 overwrite），且 restart 后要恢复。
- **C. 新 Kafka topic + trade-dump 投影**：trigger 把状态转换
...
采纳 **方案 C**：
- 新 Kafka topic `trigger-event`（key=`user_id`）
- trigger 服务在每次状态转换（PENDING → TRIGGERED / CANCELED /
  REJECTED / EXPIRED）时发 `TriggerUpdate` 事件，事件带
  **完整 post-change 快照**
- trade-dump 新增 consumer 投影到 `triggers` MySQL 表，按 `id`
  upsert，用 `last_update_ms` 做"last-write-wins"守卫
- history 服务新增 `GetTrigger` / `ListTriggers` RPC 读这张表
- BFF `GET /v1/trigger?scope=active|terminal|all` 三档路由：
  - `active` 走 trigger gRPC（热路径、无历史延迟）
  - `terminal` / `all` 走 history gRPC
  - `GET /v1/trigger/:id` 先试 trigger；NotFound 时 fall back 到 history
- `include_inactive=true` 兼容 MVP-14 行为：history 未部署时回落 trigger `IncludeInactive`

### Producer 事务性 — MVP 不做

trigger 的 journal producer **不走 Kafka 事务**。理由：
...
3. trigger 服务的 source of truth 仍是本地 JSON snapshot（ADR-0036
   模式），Kafka 出岔子不影响业务；MVP-14c 的 HA 依赖 etcd lease + 本地
   snapshot，跟 Counter/Match 的 EOS 模型（ADR-0032）不同

未来若要把 trigger 也升级到 EOS：换 `kgo.DisableIdempotentWrite()`
...
trigger 服务启动时 `atomic.Uint64` 从 0 开始，重启后也是 0。若用
...
### 方案 A: trigger 直连 MySQL

违反 ADR-0046 分层（触发单服务是写 / 触发热路径，混入读职责会增加
复杂度）。也使得 trigger 服务部署要带 MySQL 驱动 + 连接池。
拒绝。

### 方案 B: 周期性全量快照写 MySQL

写量大（每周期全表 overwrite），延迟粗（≥ 快照周期，默认 60s）。
"三个月前那笔触发单" 的需求要求秒级延迟。拒绝。
...
省 Kafka 流量（PENDING 单未被触发则无 event；绝大多数触发单最终被
cancel/expire 而不是触发）。代价是 client 看不到刚下的触发单（下单
...
- `GET /v1/trigger/:id` 的 Counter-first + history-fallback 模型
...
- 触发单终态永久可查；`trigger` 服务的 `TerminalHistoryLimit` 不再决定
  历史可见窗口
- BFF `/v1/trigger` 三档 scope 语义和订单列表统一
- 单条 `GET /v1/trigger/:id` 对已 TRIGGERED / CANCELED 等终态同
  样 200，解决 MVP-14 的 "活跃单命中、终态 NotFound" 不一致

### 负面 / 代价

- 多一个 Kafka topic；每次状态转换额外一次 produce（async，非阻塞）
- trade-dump 多一个 consumer goroutine + `trigger_events` 表
...
- `TriggerEventType` / `TriggerEventStatus` 两枚举与 rpc 层
  的同名枚举值相等，在 trade-dump / history 内做 int8 存/读

## 实施约束 (Implementation Notes)

- 新 proto `api/event/trigger_event.proto`，`TriggerUpdate`
  携带完整状态；enums 与 rpc/trigger 同值
- trigger service: `internal/journal/journal.go` 起 kgo 客户端 +
...
- trade-dump: `internal/writer/projection_cond.go` + `mysql_cond.go`
  + `internal/consumer/trigger.go`；`--trigger-topic` 空时禁
  用投影
- history: proto 新 `GetTrigger` / `ListTriggers` + cursor +
  scope=ACTIVE/TERMINAL/ALL；store/server/enums 按 MVP-15 模式镜像
- BFF: `GET /v1/trigger?scope=active|terminal|all`；
...
- [ADR-0040](0040-trigger-order-service.md) — 触发单独立服务
- [ADR-0042](0042-trigger-ha.md) — trigger HA cold standby
- [ADR-0046](0046-history-service.md) — history 只读查询聚合
- [roadmap MVP-16](../roadmap.md#mvp-16-trigger-history)
