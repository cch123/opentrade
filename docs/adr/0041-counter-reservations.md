...
[ADR-0040](./0040-conditional-order-service.md) §资金不预留（MVP-14a）
里明确给出了一条妥协：条件单不在下发时冻结资金。触发瞬间如果账户余额
已被挪作他用，Counter `PlaceOrder` 直接返回 `FailedPrecondition`，条
件单走 REJECTED。UX 很糟：止损单触发时才告诉用户 "来不及救了"。

MVP-14b 要补上这个洞，关键是把"账户余额的某一部分"在条件单下发的那一刻就
...
`/v1/reservation*` 路由，只有 conditional 服务（内网 gRPC 客户端）调
Counter.Reserve。
...
- Reservation 是 Counter 的内部 bookkeeping，它的生命周期是 "conditional 下
...
|---|---|---|
| **调用者** | 用户 / 管理员（BFF REST `/v1/transfer`） | 内部特权服务，目前只有 conditional；BFF 无对应路由 |
| **目的** | 取款冻结、合规锁仓、风控扣押 | 条件单为触发时刻预留专款 |
...
3. **journal 噪声**：每个条件单从 Place 到 Trigger 产出 1-2 条
   TransferEvent，对 trade-dump 的 account_logs 表是扰动，对账审查时
   要一直 filter `biz_type='conditional'`。分开 proto 类型 + 不发
...
### B. Conditional 不预留，触发时拿锁 + 查余额
拒绝：这是 MVP-14a，UX 差。MVP-14b 的目标就是解决这个。

### C. Counter 暴露 `PlaceOrderAtomic(reserve_and_place)`，一个 RPC 搞定
- 条件单下发时不做任何事，触发时一个 RPC 完成 "检查 + 冻结 + 下单"
- 缺点：条件单在用户端 placed 后，真正的资金锁定要到触发才发生 ——
...
### E. Reservation 走 `(asset, amount)` 全量数据，让 conditional 自己算
- conditional 不用 Counter 的 ComputeFreeze，自己预估需要冻的资产和量
- 缺点：freeze 公式的权威必须在 Counter（ADR-0035 已经反复强调），
  conditional 复制这段逻辑会造成两边漂移
- 改为 ReserveRequest 直接传 **订单 shape**（symbol/side/type/price/qty/
  quote_qty），Counter 自己跑 ComputeFreeze，就不用 conditional 关
  心 asset/amount
...
- **conditional 零知识**：conditional 的 placer 接口多了两个方法，
...
- 条件单用户体验大幅提升：下单即锁资金，触发几乎不可能因余额不足失败
...
- 代码量：proto + engine + service + server + snapshot + 4 个 conditional
  侧改 ~ 800 LOC
- trade-dump projection 在 reservation 持有期间偏高 Available：接受，
  文档化，journal-ify 留给未来
- Counter crash 无 snapshot 时会丢条件单的资金锁定 —— conditional
...
- `reservation_id` 命名空间和 `transfer_id` / `client_order_id` 完全独
  立；conditional 约定用 `"cond-<id>"`，其它服务未来接入时自己挑前缀
  避免冲突
- Reserve 走用户 sequencer，吞吐受单用户 sequencer 限制；和 Transfer /
  PlaceOrder 共享，条件单流量通常远小于下单流量，不是瓶颈
...
```json
"reservations": [
  {"user_id":"u1","ref_id":"cond-42","asset":"USDT","amount":"100","created_at_ms":...}
]
```
...
| 场景 | 结果 |
|---|---|
| Reserve 余额不足 | `ErrInsufficientAvailable`；conditional 返回 400 给用户，未存储 |
| Reserve 重复（同 ref_id） | 幂等；返回原 record 和 accepted=false |
| PlaceOrder(reservation_id) 查不到 | `ErrReservationNotFound` / NOT_FOUND；conditional 置 REJECTED |
| PlaceOrder shape 和 Reserve 不一致 | `ErrReservationMismatch` / FailedPrecondition；conditional 置 REJECTED，尝试 Release |
| Counter crash + 无 snapshot | Reservation 丢，balance 回到 pre-Reserve；conditional 触发时走 REJECTED |
| Conditional 本身 crash | Counter 保留 reservation 直到 TTL 或人工清理；14c 的 HA + 重启恢复会处理；目前留 orphan |

## 未来工作 (Future Work)

- **Journaling**：Reserve / Release 进 counter-journal，trade-dump 实时收敛
- **Reservation TTL**：`CreatedAtMs` 已经记了；后台定期清理超过 N 小时未
  消费的 orphan（防 conditional crash 永久不消费）
- **Audit endpoint**：`CounterService.ListReservations` 供运维 / 对账
- **允许 reservation 部分消费**：现在要求 exact match。如果未来支持
  修改条件单（比如改 qty）不想重新 Reserve，可以支持"消费一部分
  reservation"
...
- 实现：
  ...
  - [counter/internal/engine/reservations.go](../../counter/internal/engine/reservations.go)
  ...
  - [conditional/internal/counterclient/client.go](../../conditional/internal/counterclient/client.go)
