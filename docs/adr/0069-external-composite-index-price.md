...
- 本 ADR：把指数价升级成**多来源加权 + 离群剔除 + 失活降级**的 composite index。标记价（`mark = index + clamp(EMA(perp_mid − index), ±cap)`，EMA = exponential moving average，指数移动平均）与资金费（现为 Binance 法 premium index + 利率项 + 双重限幅，见 `markprice/internal/calc`）的公式**本 ADR 一行不改**——只是它们消费的 `index` 输入从单源换成 composite。
- 仍是单实例 + cold-standby 的轻量 `markprice` 服务，**不**单开独立 index 服务（理由见 §备选方案 E；保留未来拆分的扩展点(extension point)）。
...
- 不取的部分：参考实现还额外算了一版 mark price（基差平滑），但 opentrade 的 mark+funding 已在 `markprice` 落地且更完整（参考实现不算 funding），**指数侧才是真正缺失的部分**，故本 ADR 只接指数。
...
- **选复合**：0068 已认定自参考是结构性风险，复合是标准解法，且改动半径限于 markprice 内部。

### B. 外部源接入：拉（markprice 自己连交易所 ws）vs 推（复用现有行情管线）

- 拉（选，MVP）：markprice 内直接 ws 订阅外部所（参考实现同款），简单自洽，QPS 低（指数 1s 级）。
- 推：让 quote / 一个独立采集器把外部行情写入 Kafka，markprice 消费。更解耦但要先有外部行情采集基建，MVP 没有。
...
### E.【架构分叉点】指数计算放 markprice 内 vs 单开独立 index 服务

参考实现把 index-server 拆成**独立服务**（它还兼管对外行情订阅，所以独立有它的理由）。opentrade 是否照搬？

- **折叠进 markprice（选，MVP）**：指数和标记价同处一个 tick goroutine，省一次 topic 跳转、省一套服务运维，且 mark 本就依赖 index、二者同处一进程是自然的。QPS 低（1s 级），单实例足够。
- **独立 index 服务 + `index-price` topic**：markprice 消费 index。更解耦（指数可被 funding / 风控 / 行情展示等多方复用），但多一套 HA/snapshot/监控面 + 一跳延迟。
- **MVP 折叠**，但**把价源层隔离成 `markprice/internal/index`**（`IndexSource` 接口 + `SourceBook` + composite 计算独立可测），使得未来"指数被多方复用"时，把这个包提成独立服务 + 发 `index-price` topic 是**增量而非重写**——同 0068 对 collateral pool 扩展点(extension point)的处理思路。
