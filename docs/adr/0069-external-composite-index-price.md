...
- 现状（[ADR-0068](./0068-usdt-linear-perp.md) §5）：`markprice` 的指数价 = **自家现货单一来源**（消费现货 `market-data` 的 `BTC-USDT` 中间价）。这是 0068 自己标注的**自参考可被操纵**风险点，列在其开放问题里。
- 本 ADR：把指数价升级成**多来源加权 + 离群剔除 + 失活降级**的 composite index。标记价（`mark = index + clamp(EMA(perp_mid − index), ±cap)`）与资金费（现为 Binance 法 premium index + 利率项 + 双重限幅，见 `markprice/internal/calc`）的公式**本 ADR 一行不改**——只是它们消费的 `index` 输入从单源换成 composite。
- 仍是单实例 + cold-standby 的轻量 `markprice` 服务，**不**单开独立 index 服务（理由见 §备选方案 E；保留未来拆分的缝）。
- 参考实现：`cryptofabric/unimargin-index-server`（Java）做的就是这件事——`{"${Huobi.btcusdt}":1,"${Binance.BTCUSDT}":2,"${Okex.BTC-USDT}":1}` 加权公式 + 30s 源失活 + ≥3 源时按中位数剔 5% 离群。本 ADR 把同类逻辑落到 Go 的 `markprice` 里，并补齐参考实现缺的两件事：**最小源仲裁（quorum）**与**指数失活时冻结强平**。
...
| `degraded`（自参考降级） | 仅剩自家现货一个源时的兜底态，明确标记风险 | — |

## 背景 (Context)

### 现状（已落地，2026-05-30 核实）

`markprice/cmd/markprice/main.go` 的指数价来源是**单一的**：
...
1. **自参考是结构性风险**：perp 的未实现盈亏 + 强平判定全跑 mark，而 mark 锚在 index 上。若 index 只看自家现货 `BTC-USDT`，攻击者**砸自家现货盘口**就能传导到 perp 的 mark，触发非自然强平——0068 用"保守 MMR + 限幅"缓解，但没拔掉根。
2. **参考实现给了现成蓝图**：`unimargin-index-server` 已经把多源加权 + 离群 + 失活跑通，算法清晰可移植。
3. **改动半径极小**：只在 `markprice` 内部把"取一个 mid"换成"取一个 composite"，下游（mark/funding 公式、`mark-price` topic 格式、perp-counter）几乎不动。现在做成本最低。
...
- **缺口 1（本 ADR 补）**：没有显式 **quorum**——存活源跌到 1 个它仍会算，等于偷偷退化成自参考而不报警。
- **缺口 2（本 ADR 补）**：指数失活时它只是停发，**下游怎么办没有契约**。perp 这边必须明确：指数失活 → 标记价标 `stale` → **perp-counter 暂停强平**（强平依赖新鲜 mark，跑陈旧/兜底 mark 去平仓比不平更危险）。
- 不取的部分：参考实现还顺手算了一版 mark price（基差平滑），但 opentrade 的 mark+funding 已在 `markprice` 落地且更完整（参考实现不算 funding），**指数侧才是真正缺的那块**，故本 ADR 只接指数。

## 决策 (Decision)

### 1. composite index 算法

每个 perp symbol 配一组 `index_source`，每个源给出 `(price, ts)`。每个 markprice tick（沿用现有 `--tick-interval`，默认 1s），在**拥有 Calc 的那个单 goroutine 内**按下式算指数：
...
`index` 喂给现有 `calc.Tick(index, perpMid)`，**之后的一切（mark / funding / topic / perp-counter）不变**。

### 2. 价源与接入方式

- **自家现货源**：沿用现有现货 `market-data` Kafka 消费（`book.Mid(spotSymbol)`），作为一个 `index_source`（权重可配）。
- **外部交易所源**：在 `markprice` 内新增 `internal/index` 的 source adapter（Binance / OKX / Huobi 起步，websocket 订阅其现货 ticker/bookTicker，参考实现同款），各自维护 `(price, ts)` 写进一个并发安全的 `SourceBook`（镜像现有 `MidBook` 的写法）。
- 统一接口：
...
指数源是外部依赖，**必须显式定义"源不够时怎么办"**，否则 perp 会偷偷拿坏指数去强平。分三档：

| 存活源数 | 状态 | markprice 行为 | perp-counter 行为 |
|---|---|---|---|
| `≥ quorum` 且含 ≥1 外部源 | 正常 | 正常发 `MarkTick{index_stale=false}` | 正常结算 + 强平 |
| `≥ quorum` 但**只剩自家现货** | `degraded`（自参考兜底） | 发 `MarkTick{index_stale=false, degraded=true}` + 告警 | 正常结算 + 强平（但带降级告警，runbook 关注） |
| `< quorum` | `index_stale` | 持有**最后一次 good index**，发 `MarkTick{index_stale=true}`（mark 用冻结 index 算，仅供展示/盈亏，不可强平） | **暂停强平**该 symbol（不产生新破产单），未实现盈亏照常展示；超过硬 TTL 继续 stale 则升级告警 |

即：**强平只在指数新鲜时进行**。这是本 ADR 的核心安全不变量——宁可暂停强平（风险后置、人工可介入），不可拿陈旧/可疑指数去平用户仓位（不可逆、且正是攻击面）。

### 4. 配置形态

MVP 用 markprice 的配置承载（flag + 一个 per-symbol 的 JSON，参考实现同款），长期对齐 [ADR-0056](./0056-symbol-config-via-mysql.md) 的 SymbolConfig（MySQL 权威 + 轻量 poll）：
...
MarkTick { symbol, mark_price, index_price, funding_rate, ts,
           ...
           index_degraded // 仅剩自家现货：自参考兜底，告警但不暂停
}
```
...
- 单源（现状）：零外部依赖、零新故障面，但自参考、可被现货盘口操纵传导，且单一现货源故障 = 指数全挂。
- 复合（选）：抗操纵、抗单源故障；代价是引入外部行情依赖 + 失活/降级的运维面。
- **选复合**：0068 已认定自参考是结构性风险，复合是教科书解法，且改动半径限于 markprice 内部。

### B. 外部源接入：拉（markprice 自己连交易所 ws）vs 推（复用现有行情管线）

- 拉（选，MVP）：markprice 内直接 ws 订阅外部所（参考实现同款），简单自洽，QPS 低（指数 1s 级）。
- 推：让 quote / 一个独立采集器把外部行情灌进 Kafka，markprice 消费。更解耦但要先有外部行情采集基建，MVP 没有。
...
- 等权：配置权重的退化特例。
- 成交量加权（VWAP across exchanges）：更抗薄盘所操纵，但要实时拉各所成交量，复杂，列 future。
- **MVP 选配置权重**，存活子集上重新归一。
...
### E.【架构岔路】指数计算放 markprice 内 vs 单开独立 index 服务

参考实现把 index-server 拆成**独立服务**（它还顺带管对外行情订阅，所以独立有它的理由）。opentrade 是否照搬？

- **折叠进 markprice（选，MVP）**：指数和标记价同处一个 tick goroutine，省一次 topic 跳转、省一套服务运维，且 mark 本就依赖 index、放一起天然。QPS 低（1s 级），单实例足够。
- **独立 index 服务 + `index-price` topic**：markprice 消费 index。更解耦（指数可被 funding / 风控 / 行情展示等多方复用），但多一套 HA/snapshot/监控面 + 一跳延迟。
- **MVP 折叠**，但**把价源层隔离成 `markprice/internal/index`**（`IndexSource` 接口 + `SourceBook` + composite 计算独立可测），使得未来"指数被多方复用"时，把这个包提成独立服务 + 发 `index-price` topic 是**增量而非重写**——同 0068 对 collateral pool 缝的处理思路。

## 理由 (Rationale)

1. **只换输入不动公式**：`calc.Tick` 早已把 index 当外部输入，复合指数是纯粹的"喂什么"问题，mark/funding/topic/perp-counter 的正确性不受影响，回归面小。
2. **安全优先于可用**：指数失活时**冻结强平**而非拿坏数据硬算——强平不可逆且是攻击面，宁停勿错。这是本 ADR 相对参考实现最重要的补强。
3. **抄已验证的算法**：加权 + 失活 + 离群三件套直接对标 `unimargin-index-server` 已跑通的实现，降低设计风险；只补它缺的 quorum + 下游失活契约。
4. **留拆分缝**：价源层独立成包，未来要独立 index 服务是增量。

## 影响 (Consequences)

### 正面

- 拔掉自参考的根，perp mark/强平抗现货盘口操纵能力实质提升。
- 抗单一现货源故障：任一源挂掉，只要 ≥quorum 仍在就照常。
- 指数失活有明确、安全的降级契约（冻结强平 + 告警），不再"偷偷退化"。

### 负面 / 代价

- **引入外部行情依赖**：markprice 现在要连 Binance/OKX/Huobi 的 ws，多了一类故障面（断线、限频、协议变更）；靠失活过滤 + quorum + 降级态吸收。
- **外部源协议适配的维护成本**：各所 ws 报文格式不同，adapter 要各写各维护（参考实现已有可借鉴的解析）。
- **强平可能因指数失活而暂停**：极端情况下（多源同时挂）强平停摆，风险后置到人工——这是刻意的安全取舍，runbook 须写明监控与人工介入流程。
- 合规/法务面：拉外部交易所行情可能涉及数据使用条款，上线前需确认（非工程问题，标注待办）。

### 中性

- markprice 仍单实例 + cold-standby，QPS 不变（指数 1s 级，远低于撮合）。
...
- 新增 `markprice/internal/index`：`IndexSource` 接口、`SourceBook`（并发安全 latest 价表，镜像 `MidBook`）、`Composite(snapshot, cfg) (index, stale, degraded)` 纯函数（失活/quorum/离群/加权全在这，独立单测）。
- 外部 adapter（`internal/index/binance`、`okx`、`huobi`）：ws 订阅 + 重连 + 写 `SourceBook`，镜像参考实现的 channel adapter。
- `runTickLoop` 改一行：`spotMid` → `index := composite.Eval(...)`；据 `stale/degraded` 设置 `MarkTick` 的两个布尔位；stale 时沿用上一 good index 算 mark（仅展示）且 funding 不结算。
- perp-counter markprice 消费侧：`index_stale=true` → 跳过 `scanLiquidations(symbol)`（保留 `SetMark`），打点 + 告警。
...
- 注入式：构造"自家现货被砸但外部所不动"→ 验证复合指数不被单源带偏（抗操纵回归）。

### 附录：关键流程 ASCII 序列图

#### 图 1 — composite index 计算（源 fan-in → 过滤 → 加权 → 喂 mark）

```
 外部所 ws (binance/okx/huobi)        markprice                          perp-counter
...
- **成交量加权（cross-exchange VWAP）**：比配置权重更抗薄盘所操纵，何时引入。
...
- 实现位置（动工时涉及）：`markprice/internal/index/`（新增价源层 + composite 纯函数）、`markprice/cmd/markprice/`（tick loop 接入 + flag）、`api/event/mark_price.proto`（MarkTick 加 `index_stale` / `index_degraded`）、`perp-counter/internal/markprice/`（消费侧 stale → 暂停强平）。
