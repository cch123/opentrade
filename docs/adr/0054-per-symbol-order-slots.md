# ADR-0054: 单用户 per-symbol 挂单数上限（Limit / Trigger 双槽位）

- 状态: Accepted
- 日期: 2026-04-19
- 决策者: xargin, Claude
- 相关 ADR: 0030（match etcd symbol sharding）、0040（trigger order service）、0041（counter reservations）、0044（trigger OCO）、0045（trailing stop）、0048（snapshot/offset 原子绑定）、0052（admin console）、0053（symbol 精度治理）
...
| `MaxActiveTriggerOrders` | 单用户在该 symbol 上的**未触发触发单**上限 | Binance `MAX_NUM_ALGO_ORDERS` / OKX `maxTriggerOrders` / Bybit TP/SL 数量上限 |
...
| `未触发触发单` | trigger 服务中 `Status == TRIGGER_STATUS_ACTIVE`（尚未 triggered / canceled / expired） | BN algo orders 的 NEW 状态 |

**不计入槽位**：
- MARKET 单（无论 IOC/FOK；本身瞬时 terminal，不占 orderbook）
- trigger 触发后**派生**出的 counter LIMIT 单本身占 `MaxOpenLimitOrders`（见下文语义），但这个"派生"不在 trigger 槽位继续扣 —— trigger 触发瞬间从 pending → terminal，ALGO 槽位立刻释放
...
2. `trigger/internal/engine/engine.go:171-177` 的 `pending / byClient / lastPrice`，**无 per-(user, symbol) 索引**，List/tryFire 都全表扫
...
- **Trigger 槽位**：单用户在 symbol 上的**未触发触发单**数 ≤ `MaxActiveTriggerOrders`，默认 **10**（BN 典型值 5；MVP 留余量）
- 两个槽位**互不影响**：LIMIT 不占 trigger 额度，反之亦然
- MARKET 单**不计**任何槽位
...
- 零值 = **用全局默认**（Counter/Trigger 启动配置里的 `DefaultMaxOpenLimitOrders` = 100、`DefaultMaxActiveTriggerOrders` = 10）
...
**Trigger `Engine.Place`**（`trigger/internal/engine/engine.go:243`）：

- 校验位置：`buildTrigger` 之后、插入 `pending` 之前，仍在 `e.mu` 临界区内
- 所有 trigger type（STOP_LOSS、TAKE_PROFIT、TRAILING_STOP、OCO）一律计入
- 超限 → 返回 `ErrMaxActiveTriggerOrdersExceeded`（新增 sentinel）
...
```go
activeTriggers map[string]map[string]int // user_id → symbol → count
```
...
### 5. trigger 触发派生单的占槽行为（BN 对标）

trigger 触发时派生 counter LIMIT 单（`counterclient` 走 PlaceOrder）：

- **该派生单照常占 Counter `MaxOpenLimitOrders` 槽位**（走 PlaceOrder 正常路径，计数自然 ++）
- **不预占**：trigger `Place` 阶段**不预留** Counter 槽位
- **触发时超限**：派生 PlaceOrder 拿到 `MAX_OPEN_LIMIT_ORDERS_EXCEEDED` → trigger 服务将该单置 terminal `EXPIRED_IN_MATCH` 状态（BN 术语）
- 对应 `RejectReason` 需要在 `counterclient.PlaceOrder` 返回路径透传到 trigger engine
...
### C. Trigger 预占 Counter 槽位

- 优点：触发一定能下成，体验好
- 缺点：一个用户挂 100 个 stop 单就让该 symbol 所有新 LIMIT 报错，与"trigger 不产生市场影响"的直觉反；BN 未采用
...
- trigger 触发派生单的拒绝路径（`EXPIRED_IN_MATCH`）是新状态分支，需要 push / quote / trade-dump 侧识别
...
4. **M4**：Trigger `Engine.activeTriggers` 索引 + `Place` 超限 reject + 快照恢复重建
5. **M5**：Trigger 触发派生单的拒绝路径打通（`counterclient` 透传 RejectReason → `tryFire` 置 `EXPIRED_IN_MATCH`）
...
- Trigger：STOP + OCO + TrailingStop 混合场景下槽位共用同一计数
- 链路：trigger 触发派生单遇 Counter 满 → `EXPIRED_IN_MATCH` + push 能发出终态

### 配置降级兼容

- 既有 etcd 中不带这两字段的 SymbolConfig `Unmarshal` 仍合法（`omitempty`）
- Counter/Trigger 启动配置里**必须**提供非零全局默认，否则服务启动 fail-fast（防止 0 = 彻底禁止下单的误配）
...
- ADR-0040 / 0044 / 0045: trigger 单生命周期（graduate / cancel / expire）
