...
- 日期: 2026-04-20
- 决策者: xargin, Claude
- 相关 ADR: 0001（Kafka 作为事件权威源）、0002（Counter HA via etcd lease）、0030（match etcd symbol sharding）、0031（HA cold standby）、0042（trigger HA）、0052（admin console）、0053（symbol 精度治理）、0054（per-symbol 挂单数上限）
...
- **消费方**：match / counter / trigger / BFF 通过 `pkg/etcdcfg.EtcdSource` 的 `List + Watch` 读取并缓存到各自的 symregistry
...
CREATE TABLE symbol_config (
    ...
    max_active_trigger_orders INT UNSIGNED  NOT NULL DEFAULT 0,   -- ADR-0054
    updated_at                    TIMESTAMP(6)  NOT NULL
                                  DEFAULT CURRENT_TIMESTAMP(6)
                                  ON UPDATE CURRENT_TIMESTAMP(6),
    CHECK (max_open_limit_orders         <= 10000),
    CHECK (max_active_trigger_orders <= 10000),
...
- **ADR-0054 backlog 第 2 项** 顺手做掉：trigger 的 `SymbolLookup` 也用新 source 接上
- **ADR-0054 backlog 第 3 项** 顺手做掉：`counter/internal/symregistry` 搬到 `pkg/symregistry/`（因为 cfgstore 要提供给 match / counter / trigger 三家共用）
...
- 顺手清了 ADR-0054 的 backlog 第 2、3 项（trigger SymbolLookup + symregistry 搬家）
...
- 集成：admin-gateway PUT → 1s 内 counter / match / trigger cache 看到新值；precision rollout（ADR-0053）端到端
...
- [ADR-0054](./0054-per-symbol-order-slots.md) — max_open_limit_orders / max_active_trigger_orders 字段的业务语义
- 实现位置（动工时涉及）：
  ...
  - counter / match / trigger `cmd/*/main.go` — 切 Source
