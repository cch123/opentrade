...
**Fencing 覆盖范围**：MVP-12 rollout 里的 dual-write review 发现 Transfer 和
Settlement 原本走 non-txn `JournalProducer`（idempotent only，跨进程不
fence），所以 HA 下老主和新主都能并发写 `counter-journal` 的 Transfer /
Settlement 事件。修复方式：删除独立 `JournalProducer`，把 `TxnProducer` 的
`PublishJournalOnly` 改名为 `Publish`（匹配 `service.Publisher` interface），
Counter 的所有 `counter-journal` 写入路径（PlaceOrder / CancelOrder /
Transfer / settlement / OrderStatus）统一走同一个 txn.id，fencing 全覆盖。
