...
判据：**多模块共用**的业务公共结构进 pkg/；**单服务专属**的留服务目录。

```
pkg/snapshot/             # 通用基础设施：Format / BlobStore（FS / S3）
pkg/snapshot/counter/     # ShardSnapshot wire format（counter consumer + trade-dump producer 共用）
pkg/snapshot/trigger/     # TriggerSnapshot wire format（trigger consumer + trade-dump producer 共用）
pkg/counterstate/         # Counter state machine + apply（counter authoritative + trade-dump shadow 共用）
pkg/projection-runtime/   # 后续：consumer wrapper / writer 接口（视需要）
```

**不进 pkg/ 的**：
- 单服务自己用的辅助类型（`<service>/internal/...`）
- `<service>/internal/snapshot/` 里的 loadSnapshot 收口包装——它们 import pkg/snapshot/{服务}，但本身只对自己服务有意义

**模块依赖收成单向**：counter / trade-dump / trigger 都只依赖 api + pkg，相互之间没有 cross-module require。所有跨服务共享走 pkg/ 单向流入。

> **注**：本节定稿前曾尝试 "service-specific 一律不进 pkg" + "wire format 由 producer 持有（trade-dump/snapshot/...）"两条线。实际落地发现 counter（consumer）必须反向 import trade-dump 才能 decode snapshot——producer 反 import consumer 是糟糕的方向。改用上述"多模块共用进 pkg"判据后这条反向依赖消除。
...
- ❌ 不要把**单服务专属**的 snapshot 辅助类型搬到 pkg/snapshot/（pkg/ 只放至少两个模块共用的部分；判据见 §4）
- ❌ 不要让任一服务模块 require 另一服务模块（共享走 pkg/）
