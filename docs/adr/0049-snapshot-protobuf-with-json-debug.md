...
四个持久化内存态服务 —— counter / match / conditional / quote —— 的 snapshot 文件都是 JSON。JSON 的好处是 `jq` 能人眼排查、schema 演进宽松；代价是体积 2-3× 于等价 protobuf、encode/decode 占用 CPU。
...
- 批 3b-2/3/4 (match / conditional / quote)：同模式复制，不再写新 ADR（ADR-0049 统一管辖）
