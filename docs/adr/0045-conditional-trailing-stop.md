...
上。本 ADR 给 conditional 服务增加一个显式的 `TRAILING_STOP_LOSS` 类型，
保留后续演化空间。

## 决策 (Decision)

### 1. 新 `CONDITIONAL_TYPE_TRAILING_STOP_LOSS = 5`（仅 MARKET）
...
Conditional 多 4 个可变状态字段：`TrailingDeltaBps`、`ActivationPrice`、
`TrailingWatermark`、`TrailingActive`。

每条 PublicTrade 到达 → 引擎在锁内对所有 trailing 的 pending 条件单跑：
...
- `api/rpc/conditional/conditional.proto`：
  - Enum 加 `CONDITIONAL_TYPE_TRAILING_STOP_LOSS = 5`
  - PlaceConditionalRequest 加 `trailing_delta_bps` / `activation_price`
  - Conditional 加 `trailing_delta_bps` / `activation_price` /
    `trailing_watermark` / `trailing_active`
- `conditional/internal/engine/engine.go`：
  - Conditional 4 个字段
  - buildConditional：trailing 分支 + 5 个新 err
...
- snapshot/snapshot.go：ConditionalSnap 加 4 字段；Capture / Restore
  覆盖；parse 错误带 id 上下文
- server/server.go：5 个新 err 映射到 InvalidArgument
- bff/internal/rest/conditional.go：
  - `placeConditionalBody` 加 `trailing_delta_bps` / `activation_price`
  - parseConditionalType / conditionalTypeLabel 加 `trailing_stop_loss`
  - Place handler + OCO leg 都透传
  - conditionalToJSON 暴露四个字段 + oco_group_id
