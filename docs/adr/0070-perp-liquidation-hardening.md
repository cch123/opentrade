...
3. **兜底接管（backstop）**：盘口吃不下破产单时的保证成交机制（对标参考实现 BURST / 做市商接管），让仓位**总能平掉**。
...
参考实现：`cryptofabric/unimargin-liquidate-server`（Java，Raft 副本的独立服务）有完整的 6 策略阶梯（Offset/Reduce/Force/Burst/Agreement/Collateral）+ 阶梯 MMR。本 ADR **取其策略逻辑、不取其架构**——它的 Raft + 独立服务恰恰是 0068 §C 论证过的"独立强平服务读仓位 = TOCTOU"反面教材（见 §备选方案 E）。
...
| `backstop`（兜底接管方） | 盘口吃不下时保证成交的系统账户；其库存风险由风险台/保险基金承接 | 参考实现 market-maker account / 三家"接管方" |
| `ADL`（自动减仓） | 保险基金兜不住穿仓时，强制平掉对手方盈利仓补足 | 三家 auto-deleveraging |
...
[ADR-0068 §8/§9](./0068-usdt-linear-perp.md) 的强平 MVP 已跑通（`perp-counter/internal/service/liquidation.go` + `engine/engine.go` + `pkg/perpstate/`）：
...
4. **无兜底**：破产单挂在 Match 盘口靠自然流动性；MVP 注释明确"若流动性不足只部分成交，剩余不自动再挂（在途 guard 持有，下个 tick 跳过）"——**盘口薄 → 仓位卡住不平**。
...
0068 的 MVP 在"保守 MMR + 小概率穿仓人工介入"假设下能跑，但有三个**会真出事**的缺口：

- **缺口 A（盘口薄 → 卡仓）**：现状破产单挂盘口，吃不下就卡着，下个 tick 跳过。行情急跌时盘口本就薄，**最该平的时候最平不掉**，亏损扩大、穿透保险基金。这是最危险的缺口。
- **缺口 B（整仓强平过度 + 冲击放大）**：现状一律整仓平。仓位只是轻微破 MM 时，整仓砸盘既伤用户（本可只平一点）又放大市场冲击（自激下跌）。部分强平是行业标准做法。
- **缺口 C（ADL 不执行 → 穿仓无解）**：基金为负只告警，没有自动补足机制。真发生穿仓且无人值守 = 系统资不抵债。
- 阶梯 MMR 是 A/B/C 的共同地基（破产价、强平触发、最大杠杆都依赖它）。

## 决策 (Decision)

### 1. 阶梯风险限额（risk tier）—— 地基
...
落地最关键的一点——**保住 0068 不变量 #6（collateral pool 缝）**：现状 `pool.Liquidatable(marks, mmr)` 吃标量，改成吃一个**MMR 解析函数**：
...
破 MM 时不再无脑整仓平，先算**平回安全线所需的最小数量** `q*`：
...
- **退化为整仓**：当 `q*` ≥ 剩余仓位（权益已不足以靠部分平回安全线），或剩余 < 最小 lot → 直接整仓走 §3 破产价接管。

即强平变成**单调逼近**：每 tick 平掉"恰好够回到安全线"的量，要么收敛回安全、要么逼近到整仓接管。

### 3. 兜底接管（backstop）—— 拔掉"盘口薄就卡仓"

强平单（§2 的 liq_price 部分单 / 整仓破产单）走**升级阶梯**，保证仓位**总能在有限步内平掉**：

```
(a) 挂 reduce_only @ liq_price 到 Match —— 正常情况盘口自然吃掉
(b) N 个 tick 内未成交/仅部分成交 → 撤剩余, 重挂 @ bankruptcy_price（更激进, 多数情况这就成交了）
(c) 仍吃不下 且 mark 已越过 bankruptcy_price → backstop 接管：
...
- backstop = 一个系统账户（`--backstop-account`）。被强平仓在内部按破产价平掉，**对侧库存记到 backstop 账户**（它现在背了一个反向仓），权益缺口由保险基金结算（成交优于破产价→盈余进基金；劣于→基金补）。
- backstop 账户的库存风险由**链下风险台 / 做市商对冲**（与参考实现把仓位甩给做市商账户同构）；perp-counter 只保证"用户仓位一定平掉 + 账目守恒 + 出告警"，不在链上消化库存风险。
- backstop 接管是**保证闭合的最后一环**，永不"静默跳过"——这是相对 0068 现状"剩余不再挂"的根本修复。

> 为什么 (c) 走内部对平而非"系统账户去 Match 挂单"：再挂 Match 仍可能吃不下（盘口是空的），无法保证闭合；内部对平在 sequencer 内一步完成、确定闭合、账目守恒，且天然无 TOCTOU。代价是 backstop 账户承接库存（需链下对冲），这是"保证闭合"的必要对价。
...
4. 每个被 ADL 的对手仓: 在它自己的 sequencer 内平仓, 已实现盈亏按破产价落账, 发 PerpAdlEvent 通知该用户
```

- ADL 把穿透的亏损**确定性地分摊给盈利方**（赢家让出部分利润），这是基金兜不住时维持系统偿付能力的最后手段，行业通用。
- **仅当 `deficit > 0`（基金真兜不住）才 ADL**；基金够补则不动对手仓（不变量）。

#### 跨 sequencer 的 ADL hand-off（本 ADR 最敏感的并发点）

触发强平跑在**穿仓用户 A 的 sequencer** 里，但被 ADL 的对手仓属于**用户 B/C/...**——动它们必须进**它们各自的 sequencer**（0068 不变量 #1：一个用户的所有仓位 mutation 只在该用户 sequencer 内串行）。绝不能在 A 的 sequencer 里直接改 B 的仓位（那就是 0068 §C 批判的 TOCTOU）。
...
mark tick → pool health 破 MM ?
  ...
  是 → 进强平:
       ◇ 权益 > 用部分平回安全线所需 ?
         ...
         否 → 整仓接管:
              挂 @ bankruptcy_price → (盘口吃不下) → backstop 内部对平（§3）
              ◇ 保险基金转负 (deficit>0) ?
                否 → 完成（盈余/小亏由基金吸收）
...
- 仅整仓（现状）：实现简单，但破 MM 即整仓砸盘——伤用户（本可少平）、放大市场冲击（自激）、穿仓概率更高。
- 部分强平（选）：只平回安全线，用户保残仓、市场冲击小，行业标准。代价：求解 `q*`（含跨档）+ 多 tick 复查的复杂度，多笔强平单。
- **选部分强平**，整仓接管作为权益不足时的退化分支保留。

### B. 兜底机制：靠自然盘口（现状）vs 重挂逼近 vs backstop 内部对平 vs 外部做市商

- 自然盘口（现状）：吃不下就卡仓，**最危险**，否决。
- 重挂逼近破产价（升级阶梯 b）：多数情况够用，但盘口真空时仍不闭合。
- backstop 内部对平（选，终局）：保证闭合、无 TOCTOU、账目守恒；代价是 backstop 账户承接库存、需链下对冲。
- 外部做市商账户去 Match 挂单（参考实现 BURST 形态）：把库存甩给做市商，但仍走 Match、仍可能吃不下、且引入外部依赖。
- **选 backstop 内部对平作终局保证**，重挂逼近作中间档；库存对冲链下解决。

### C. ADL：自动执行（选）vs 仅告警（现状）

- 仅告警（现状）：穿仓时无自动补足，资不抵债靠人工，无人值守即风险敞口。
- 自动执行（选）：确定性分摊给盈利方，维持偿付能力。代价：跨 sequencer 协调 + 减赢家仓的用户体验/沟通成本。
- **选自动执行**，且只在基金真兜不住（deficit>0）时触发，最小化对盈利方的打扰。
...
### E.【架构岔路，重申】强平内置 perp-counter vs 独立强平服务（参考实现形态）

参考实现是**独立服务 + JRaft 副本**，opentrade 是否照搬？
...
- 内置 perp-counter（选，0068 §C 已定，本 ADR 重申）：强平判定/执行与仓位真值同处一个 sequencer，无跨服务读、无 TOCTOU；HA 靠现成的 snapshot 绑 offset（[ADR-0048](./0048-snapshot-offset-atomicity.md)）+ 事务 producer fencing（ADR-0032）+ cold-standby（ADR-0031），**不需要再引入一套 Raft**。
...
- 硬编码 / 文件：MVP 省事但不可运维热调，否决。
- **选 SymbolConfig**，档表作为 perp symbol 的一组字段。

## 理由 (Rationale)

1. **拔掉最危险的"卡仓"缺口**：backstop 保证闭合，把"行情急跌盘口薄→最该平时平不掉"这个会真实造成系统性亏损的洞补上。
2. **部分强平是风控正道**：少平、少冲击、少穿仓，且对用户友好——行业标准，不是 over-engineering。
3. **ADL 让穿仓有终局解**：确定性分摊优于资不抵债，且只在基金兜不住时启用。
4. **阶梯 MMR 是地基且不破 collateral pool 缝**：用 `MMRFunc` 穿过 pool 接口，保住 0068 不变量 #6，cross 仍是增量。
5. **架构不变、正确性论证不变**：全部内置 sequencer，重用现成 snapshot/HA；唯一的跨 user 协调（ADL）也在同进程内做，不退回 0068 §C 否决过的"独立强平服务 + Raft"。
6. **抄参考实现的策略、弃其架构**：策略阶梯（Reduce/Force/Burst/Agreement）逻辑成熟可借鉴，但它的 Raft + 独立服务是为"externalized 强平状态"付的税，opentrade 内置后不必付。

## 影响 (Consequences)

### 正面

- 强平在薄盘下也**保证闭合**，堵住最大的系统性亏损敞口。
...
- **复杂度与正确性敏感度显著上升**：部分强平 `q*` 求解（含跨档）、升级阶梯状态机、ADL 跨 sequencer 协调——都是钱直接相关 + 顺序/并发敏感，测试成本高（见 §测试要点）。
- **backstop 引入库存风险**：系统账户承接被接管仓位的反向库存，须有链下对冲机制兜住，否则风险只是从用户转移到平台账面（须 runbook + 风险台流程）。
- **ADL 影响盈利用户体验**：被减仓的赢家会让出利润，须有清晰的事前规则披露 + 事后通知（PerpAdlEvent → push/站内信），否则是信任事件。
- **snapshot / 恢复面变大**：部分强平进度、升级阶梯计时、backstop 库存、ADL 在途队列都要进 snapshot（0068 不变量 #5 扩展），否则崩溃恢复不一致。

### 中性

- 仍单实例 / 小分片起步；ADL/backstop 是低频事件（极端行情才触发）。
- 仍 isolated margin；cross 的级联强平是另一 ADR，本 ADR 的 pool 接口已为其留缝。
...
9. **backstop 保证闭合**：整仓接管在有限步内必定闭合（盘口→重挂→内部对平兜底），**永不静默跳过**；backstop 库存 + 保险 delta 必入账 + 出告警。
10. **ADL 仅在基金兜不住时触发**（deficit>0）；每个被 ADL 对手仓在**其自身 sequencer 内**复核仍反向/盈利/存在后才平（TOCTOU）；按 `last_match_seq`/`adl_round` 幂等，重放不重复减仓。
...
- 集成：开仓→mark 渐跌→**部分强平多次逼近**→价格继续跌→整仓 backstop 接管→基金转负→**ADL 减对手盈利仓**全链路；空盘口下整仓仍闭合；perp-counter 在部分强平/ADL 中途宕机→snapshot+replay 恢复一致（在途阶梯 + ADL 队列不丢不重）。
...
perp-counter
 user A sequencer (穿仓触发方)            内部任务派发              user B/C sequencer (对手盈利方)
   ...
   │ 选出名单(只读快照, 不动 B/C 仓位) ───►│ 逐个投递 ADL 任务 ─────►│ (进 B 的 sequencer)
   │                                       │                         │ TOCTOU 复核: 仍反向/盈利/存在?
...
- **backstop 库存的链下对冲流程**：风险台/做市商如何接走 backstop 账户库存，对冲 SLA 与告警阈值（运维 + 业务，非纯工程）。
- **ADL 通知与披露**：事前规则披露 + 事后 `PerpAdlEvent` 通知形态；并列 adl_score 的确定性打破规则。
- **socialized loss 基金 vs ADL 的边界**：是否引入独立的"社会化损失分摊"作为 ADL 之外的中间层。
- **cross / 统一保证金下的级联强平**：pool 接口已留缝，级联选择 + 跨 symbol 风险单元是单独 ADR。
...
- 参考实现：`cryptofabric/unimargin-liquidate-server`（Java，Raft 独立服务）—— 取其 Reduce/Force/Burst/Agreement 策略逻辑，弃其 Raft + 独立服务架构（其 Raft 复杂度正是 0068 §C 批判的 externalized 强平状态的代价）。
