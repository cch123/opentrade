...
// ADR-0054: Engine.activeTriggers caps pending triggers per
...
func TestTriggerCap_DefaultEnforced(t *testing.T) {
...
func TestTriggerCap_CancelFreesSlot(t *testing.T) {
...
func TestTriggerCap_SymbolOverrideTightens(t *testing.T) {
	lookup := SymbolLookup(func(sym string) (etcdcfg.SymbolConfig, bool) {
		if sym == "BTC-USDT" {
			return etcdcfg.SymbolConfig{MaxActiveTriggerOrders: 1}, true
		}
	...
	if _, _, ok, err := e.Place(context.Background(), goodReq()); ok || !errors.Is(err, ErrMaxActiveTriggerOrdersExceeded) {
		t.Fatalf("symbol override cap should block: ok=%v err=%v", ok, err)
	}
}

func TestTriggerCap_ZeroDisablesCheck(t *testing.T) {
...
// FailedPrecondition gRPC status; the engine must land the trigger
...
func TestTryFire_CounterMaxOpenLimitOrders_RoutesToExpiredInMatch(t *testing.T) {
	...
	if got.Status != condrpc.TriggerStatus_TRIGGER_STATUS_EXPIRED_IN_MATCH {
