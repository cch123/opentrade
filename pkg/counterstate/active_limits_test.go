...
func newLimit(id uint64, user uint64, sym string, status OrderStatus) *Order {
	return &Order{
		ID:     id,
		UserID: user,
		Symbol: sym,
...
func TestActiveLimits_InsertIncrements(t *testing.T) {
	s := newOrderStore()
	if err := s.Insert(newLimit(1, 1001, "BTC-USDT", OrderStatusPendingNew)); err != nil {
		t.Fatal(err)
	}
	if got := s.CountActiveLimits(1001, "BTC-USDT"); got != 1 {
		t.Errorf("count = %d, want 1", got)
	}
	if got := s.CountActiveLimits(1001, "ETH-USDT"); got != 0 {
		t.Errorf("other symbol leak: %d", got)
	}
	if got := s.CountActiveLimits(1002, "BTC-USDT"); got != 0 {
...
func TestActiveLimits_MarketOrdersNotCounted(t *testing.T) {
	s := newOrderStore()
	mkt := &Order{
		ID: 1, UserID: 1001, Symbol: "BTC-USDT",
	...
	if got := s.CountActiveLimits(1001, "BTC-USDT"); got != 0 {
...
func TestActiveLimits_PartialFillPreservesCount(t *testing.T) {
	s := newOrderStore()
	_ = s.Insert(newLimit(1, 1001, "BTC-USDT", OrderStatusPendingNew))
...
func TestActiveLimits_TerminalDecrementsAndCleansEmptyKeys(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if _, ok := s.activeLimits[1001]; ok {
...
func TestActiveLimits_RestoreInsertRebuildsCount(t *testing.T) {
	...
	s := newOrderStore()
	s.RestoreInsert(newLimit(1, 1001, "BTC-USDT", OrderStatusNew))
	s.RestoreInsert(newLimit(2, 1001, "BTC-USDT", OrderStatusPartiallyFilled))
	s.RestoreInsert(newLimit(3, 1001, "BTC-USDT", OrderStatusFilled)) // terminal → skip
	s.RestoreInsert(newLimit(4, 1001, "ETH-USDT", OrderStatusNew))
	s.RestoreInsert(newLimit(5, 1002, "BTC-USDT", OrderStatusPendingCancel))

	if got := s.CountActiveLimits(1001, "BTC-USDT"); got != 2 {
		t.Errorf("u1 BTC got %d, want 2", got)
	}
	if got := s.CountActiveLimits(1001, "ETH-USDT"); got != 1 {
		t.Errorf("u1 ETH got %d, want 1", got)
	}
	if got := s.CountActiveLimits(1002, "BTC-USDT"); got != 1 {
...
func TestActiveLimits_MultipleOrdersSameUserSymbol(t *testing.T) {
	...
	for i := uint64(1); i <= 5; i++ {
		o := newLimit(i, 1001, "BTC-USDT", OrderStatusPendingNew)
		if err := s.Insert(o); err != nil {
			t.Fatal(err)
		}
	}
	if got := s.CountActiveLimits(1001, "BTC-USDT"); got != 5 {
	...
	if got := s.CountActiveLimits(1001, "BTC-USDT"); got != 3 {
